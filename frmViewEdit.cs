...
namespace WinAGI.Editor {
    public partial class frmViewEdit : ClipboardMonitor, IMessageFilter {
        ...
        private readonly Stack<ViewUndo> UndoCol = [];
        private readonly Bitmap PlayImage;
        private readonly Bitmap StopImage;
        private readonly Bitmap AddCelImage;
        private readonly Bitmap AddLoopImage;
        ...
        public frmViewEdit() {
            ...
            hsbCel.Cursor = Cursors.Arrow;
            vsbCel.Cursor = Cursors.Arrow;
            InitFonts();
            ...
            hsbCel.Minimum = -VE_MARGIN;
            vsbCel.Minimum = -VE_MARGIN;
            hsbPreview.Minimum = -VE_MARGIN;
            vsbPreview.Minimum = -VE_MARGIN;
            ...
            HAlign = WinAGISettings.ViewAlignH.Value;
            tspAlignH.Image = tspAlignH.DropDownItems[HAlign].Image;
            VAlign = WinAGISettings.ViewAlignV.Value;
            tspAlignV.Image = tspAlignV.DropDownItems[VAlign].Image;
        ...
        private void frmViewEdit_HelpRequested(object sender, HelpEventArgs hlpevent) {
            ShowHelp();
        ...
        private void contextMenuStrip1_Opening(object sender, CancelEventArgs e) {
            if (picCel.Focused) {
                if (CurrentOperation == ViewEditOperation.Draw) {
                    // if drawing with right mouse button, do not open
                    // the context menu
                    e.Cancel = true;
                    return;
                }
                // floodfill doesn't set op, but should also cancel the menu
                if (SelectedTool == ViewEditToolType.Paint) {
                    e.Cancel = true;
                    return;
                }

                if (CurrentOperation != ViewEditOperation.None) {
                    // cancel any other operation
                    CurrentOperation = ViewEditOperation.None;
                }
            }
            SetEditMenu();
        }
        ...
        private void SetEditMenu() {
            ...
            case ViewEditMode.Cel:
                ...
                mnuUnselect.Visible = SelectionVisible;
                mnuFlipH.Visible = true;
                mnuFlipV.Visible = true;
                if (SelectionVisible) {
                    mnuCut.Enabled = true;
        ...
        private void mnuUndo_Click(object sender, EventArgs e) {
            ...
            case ClearView:
                EditView.CloneFrom(NextUndo.View);
                UpdateTree();
                SelectCel(0, 0);
                break;
            ...
            case ChangeSize:
                ...
                EditView[SelectedLoop][SelectedCel].Width = (byte)NextUndo.UndoData[0];
                EditView[SelectedLoop][SelectedCel].Height = (byte)NextUndo.UndoData[1];
                // recover data if new size is larger than old size
                if (NextUndo.CelData.Length != 0) {
                    EditView[SelectedLoop][SelectedCel].AllCelData = (byte[,])NextUndo.CelData.Clone();
                }
                SelectCel(SelectedLoop, SelectedCel, true);
                break;
            case Mirror:
            ...
            case ViewUndo.ViewUndoAction.FlipSelectionH:
                ...
                SelectionVisible = true;
                tmrSelect.Enabled = true;
                if (SelectedTool != ViewEditToolType.SelectArea) {
                    SelectedTool = ViewEditToolType.SelectArea;
        ...
        private void mnuCopy_Click(object sender, EventArgs e) {
            ...
            case ViewEditMode.View:
                ...
                // and caption
                frmNew.Text = VIEW_EDITOR + frmNew.EditView.ID;
                // select first cel
                SelectCel(0, 0);
                // switch to new form
                frmNew.Show();
        ...
        private void cmViewDesc_Opening(object sender, CancelEventArgs e) {
            mnuVDUndo.Enabled = EditTextBox.CanUndo;
            mnuVDCut.Enabled = EditTextBox.SelectionLength > 0;
            mnuVDCopy.Enabled = EditTextBox.SelectionLength > 0;
            mnuVDPaste.Enabled = Clipboard.ContainsText();
            mnuVDDelete.Enabled = EditTextBox.SelectionLength > 0;
            mnuVDCharMap.Visible = true;
            mnuVDSelectAll.Enabled = EditTextBox.TextLength > 0;
        }

        private void cmViewDesc_Closed(object sender, ToolStripDropDownClosedEventArgs e) {
            mnuVDUndo.Enabled = true;
            mnuVDCut.Enabled = true;
            mnuVDCopy.Enabled = true;
            mnuVDPaste.Enabled = true;
            mnuVDDelete.Enabled = true;
            mnuVDSelectAll.Enabled = true;
        }

        private void mnuVDUndo_Click(object sender, EventArgs e) {
            if (EditTextBox.CanUndo) {
                EditTextBox.Undo();
            }
        }

        private void mnuVDCut_Click(object sender, EventArgs e) {
            if (EditTextBox.SelectionLength > 0) {
                EditTextBox.Cut();
            }
        }

        private void mnuVDCopy_Click(object sender, EventArgs e) {
            if (EditTextBox.SelectionLength > 0) {
                EditTextBox.Copy();
            }
        }

        private void mnuVDPaste_Click(object sender, EventArgs e) {
        ...
        private void mnuVDDelete_Click(object sender, EventArgs e) {
            if (EditTextBox.SelectionLength > 0) {
        ...
        private void mnuVDCancel_Click(object sender, EventArgs e) {
            EditTextBox.Hide();
        }
        #endregion
        ...
        private void tsbTool_DropDownItemClicked(object sender, ToolStripItemClickedEventArgs e) {
            ...
            case ViewEditToolType.Edit:
                ToolCursor = ViewCursor.Edit;
                break;
            case ViewEditToolType.SelectArea:
        ...
        private void tspAlignH_DropDownOpening(object sender, EventArgs e) {
            ...
            ((ToolStripMenuItem)tspAlignH.DropDownItems[HAlign]).Checked = true;
        }
        ...
        private void tspAlignV_DropDownOpening(object sender, EventArgs e) {
            ...
            ((ToolStripMenuItem)tspAlignV.DropDownItems[VAlign]).Checked = true;
        }
        ...
        private void picCel_MouseDown(object sender, MouseEventArgs e) {
            ...
            // starts a drag so check for force drag first
            ...
            // edge drag to resize overrides other modes
            //!! what if I move reszing check outside of picCel?

            // action depends on tool status
            switch (SelectedTool) {
            case ViewEditToolType.Edit:
            ...
            case ViewEditToolType.SelectArea:
                if (e.Button == MouseButtons.Left) {
                    ...
                    if (SelectionVisible && Selection.Bounds.Contains(ViewPt)) {
                        ...
                        CurrentOperation = ViewEditOperation.SetSelection;
                        HideSelection();
                        mX = e.X;
                        mY = e.Y;
            ...
            case ViewEditToolType.Erase:
                ...
                // set operation
                CurrentOperation = ViewEditOperation.Draw;
                DisplayCel();
        ...
        private void picCel_MouseMove(object sender, MouseEventArgs e) {
            ...
            // some mouse movement
            if (CurrentOperation == ViewEditOperation.SetSelection && !SelectionVisible) {
                if (mX != e.X || mY != e.Y) {
                    SelectionVisible = true;
                    tmrSelect.Enabled = true;
            ...
            case ViewEditOperation.MoveSelection:
            ...
            spCurX.Text = "X: " + tmpPt.X;
            spCurY.Text = "Y: " + tmpPt.Y;
            // set cursor to provide feedback on current operation
            switch (CurrentOperation) {
            case ViewEditOperation.None:
                switch (SelectedTool) {
                case ViewEditToolType.Edit:
                    break;
                case ViewEditToolType.SelectArea:
                ...
                break;
            case ViewEditOperation.SetSelection:
                // select region defined by current position and anchor
                SelectRegion(ViewPt);
                break;
            case ViewEditOperation.MoveSelection:
                MoveSelection(new(tmpPt.X - CelOffset.X, tmpPt.Y - CelOffset.Y));
                break;
            case ViewEditOperation.Draw:
                switch (SelectedTool) {
                case ViewEditToolType.Draw:
                case ViewEditToolType.Erase:
                    // save pixel data
                    PixelInfo pixel = new() {
                        Location = ViewPt,
                        Value = EditView[SelectedLoop][SelectedCel][tmpPt.X, tmpPt.Y]
                    };
                    PixelData.Add(pixel);
                    // update cel data
                    EditView[SelectedLoop][SelectedCel][tmpPt.X, tmpPt.Y] = (byte)DrawCol;
                    DisplayCel();
                    if (ShowPreview && PreviewCel == SelectedCel) {
                        picPreview.Invalidate();
                    }
        ...
        private void picCel_MouseUp(object sender, MouseEventArgs e) {
            ...
            ViewPt.X = (int)(e.Location.X / (2 * ScaleFactor));
            ViewPt.Y = (int)(e.Location.Y / ScaleFactor);
            if (CurrentOperation != ViewEditOperation.MoveSelection) {
        ...
        private void splitCanvas_Panel1_MouseDown(object sender, MouseEventArgs e) {
            bool onright = (e.Location.X + hsbCel.Value) >= picCel.Width &&
                (e.Location.X + hsbCel.Value) < picCel.Width + VE_MARGIN &&
                (e.Location.Y + vsbCel.Value) < picCel.Height + VE_MARGIN;
            bool onbottom = (e.Location.Y + vsbCel.Value) >= picCel.Height &&
                (e.Location.Y + vsbCel.Value) < picCel.Height + VE_MARGIN &&
                (e.Location.X + hsbCel.Value) < picCel.Width + VE_MARGIN;
            ...
            if (e.Button == MouseButtons.Left) {
                ...
                if (onbottom) {
                    ...
                    CurrentOperation = ViewEditOperation.ChangeHeight;
                }
                if (onright) {
                    // at right edge - change width
                    // (or both, if height is also being changed)
                    ChangingWidth = EditView[SelectedLoop][SelectedCel].Width;
                    if (CurrentOperation == ViewEditOperation.ChangeHeight) {
                        CurrentOperation = ViewEditOperation.ChangeBoth;
                    }
                    else {
                        CurrentOperation = ViewEditOperation.ChangeWidth;
                    }
                }
            }
        }

        private void splitCanvas_Panel1_MouseMove(object sender, MouseEventArgs e) {
            bool onright = (e.Location.X + hsbCel.Value) >= picCel.Width &&
                (e.Location.X + hsbCel.Value) < picCel.Width + VE_MARGIN &&
                (e.Location.Y + vsbCel.Value) < picCel.Height + VE_MARGIN;
            bool onbottom = (e.Location.Y + vsbCel.Value) >= picCel.Height &&
            ...
            // check for edge dragging to resize cel

            // calculate agi coordinates
            switch (CurrentOperation) {
            case ViewEditOperation.ChangeWidth:
            case ViewEditOperation.ChangeHeight:
            case ViewEditOperation.ChangeBoth:
                Point tmpPt = new(0, 0) {
                    X = (int)((e.X - 3) / (2 * ScaleFactor)),
                    Y = (int)((e.Y - 3) / ScaleFactor)
                };
                ...
                case ViewEditOperation.ChangeWidth:
                    // set width
                    picCel.Width = (int)((tmpPt.X + 1) * 2 * ScaleFactor);
                    ChangingWidth = tmpPt.X + 1;
                    propertyGrid1.Refresh();
                    break;
                case ViewEditOperation.ChangeHeight:
                    // set height
                    picCel.Height = (int)((tmpPt.Y + 1) * ScaleFactor);
                    ChangingHeight = tmpPt.Y + 1;
                    propertyGrid1.Refresh();
                    break;
                case ViewEditOperation.ChangeBoth:
                    // set both
            ...
            default:
                if (onbottom || onright) {
                    // check for corner
                    if (onbottom && onright) {
                        splitCanvas.Panel1.Cursor = Cursors.SizeNWSE;
                    }
                    else if (onright) {
                        splitCanvas.Panel1.Cursor = Cursors.SizeWE;
                    }
                    else {
                        splitCanvas.Panel1.Cursor = Cursors.SizeNS;
                    }
                }
                else {
                    splitCanvas.Panel1.Cursor = Cursors.Default;
                }
                break;
            }
        }

        private void splitCanvas_Panel1_MouseUp(object sender, MouseEventArgs e) {
            switch (CurrentOperation) {
            case ViewEditOperation.ChangeWidth:
            ...
            case ViewEditOperation.ChangeHeight:
                ...
                if (EditView[SelectedLoop][SelectedCel].Height != picCel.Height / ScaleFactor) {
                    // set new height
                    ChangeHeight((int)(picCel.Height / ScaleFactor));
                }
                break;
            case ViewEditOperation.ChangeBoth:
                // if either dimension has changed
                if (EditView[SelectedLoop][SelectedCel].Width != picCel.Width / (ScaleFactor * 2) ||
                    EditView[SelectedLoop][SelectedCel].Height != picCel.Height / ScaleFactor) {
                    // set new size
                    ChangeCelSize((int)(picCel.Width / (ScaleFactor * 2)), (int)(picCel.Height / ScaleFactor));
                }
                break;
            }
            // always reset operation
        ...
        private void picPalette_Paint(object sender, PaintEventArgs e) {
            ...
            float width = picPalette.Width / 8;
            ...
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 8; j++) {
                    ...
                    g.FillRectangle(new SolidBrush(color), j * width, i * dblHeight, width, dblHeight);
                }
            }
            Color textcolor;
            ...
            g.DrawString("L", font, new SolidBrush(textcolor), width * ((int)LeftColor % 8) + 3, 17 * ((int)LeftColor / 8));
            ...
            g.DrawString("R", font, new SolidBrush(textcolor), width * (((int)RightColor % 8) + 1) - 13, 17 * ((int)RightColor / 8));
            ...
            if (SelectedLoop >= 0 && SelectedCel >= 0) {
                ...
                AGIColorIndex transpcolor = EditView[SelectedLoop][SelectedCel].TransColor;
                if ((int)transpcolor > 9) {
                    textcolor = Color.Black;
                }
                else {
                    textcolor = Color.White;
                ...
                g.DrawString("T", font, new SolidBrush(textcolor),
                    width * ((int)transpcolor % 8) + width / 2 - 3,
                    17 * ((int)transpcolor / 8));
            }
        }
        ...
        public bool LoadView(Engine.View loadview, int StartLoop = 0, int StartCel = 0, bool quiet = false) {
            ...
            if (!InGame && EditView.ID == "NewView") {
                ...
                IsChanged = EditView.Warnings != 0;
            }
            Text = VIEW_EDITOR;
            if (InGame) {
                Text += ResourceName(EditView, InGame, true);
            }
            else {
                if (EditView.ResFile.Length > 0) {
                    Text += CompactPath(EditView.ResFile, 75);
                }
                else {
                    Text += EditView.ID;
                }
            ...
            tsbPaste.Enabled = ClipboardHasCel() || Clipboard.ContainsImage();
            // check for mirrors if this is a v2089 view
            if (InGame && EditGame.InterpreterVersion.Index == AGIVersion.v2089 &&
                WinAGISettings.Warn2089Mirror.Value &&
                (EditView.Warnings & 2) == 2) {
                bool mirrored = false;
                MsgBoxEx.Show(MDIMain,
                    "This view contains loops marked as mirrored. Version 2.089 " +
                    "does not include support for mirrors. Both loops will use the " +
                    "same data without flipping the cel.\n\nYou will need to manually " +
                    "flip one of the loops and save the view if you want the loops " +
                    "to be appear mirrored.",
                    "Mirror Loops not Supported in v2.089",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Information,
                    "Do not show this warning again.", ref mirrored,
                    WinAGIHelp, "htm\\winagi\\managingviews.htm#mirrored");
                WinAGISettings.Warn2089Mirror.Value = !mirrored;
                if (!WinAGISettings.Warn2089Mirror.Value) {
                    WinAGISettings.Warn2089Mirror.WriteSetting(WinAGISettingsFile);
                }
            }
            return true;
        }
        ...
        public void SaveView() {
            if (InGame) {
                MDIMain.UseWaitCursor = true;
                bool loaded = EditGame.Views[ViewNumber].Loaded;
                if (!loaded) {
            ...
            if (InGame) {
                if (WinAGISettings.AskExport.Value) {
                    rtn = MsgBoxEx.Show(MDIMain,
                        ...
                        "Export View Before Removal", ref dontAsk);
                ...
                case DialogResult.Yes:
                    ...
                    exportName = NewResourceName(EditView, InGame);
                    if (exportName.Length > 0) {
                        EditView.Export(exportName);
                    }
                    break;
                ...
                string id;
                if (EditView.ResFile.Length > 0) {
                    id = Path.GetFileNameWithoutExtension(EditView.ResFile);
                }
                else {
                    id = "";
                }
        ...
        public void RenumberView() {
            ...
            if (NewResNum != ViewNumber) {
                ...
                Text = VIEW_EDITOR + ResourceName(EditView, InGame, true);
                if (IsChanged) {
                    Text = CHG_MARKER + Text;
                }
                if (EditView.ID != oldid) {
                    if (File.Exists(Path.Combine(EditGame.SrcResDir, oldid + ".agv"))) {
                        SafeFileMove(Path.Combine(EditGame.SrcResDir, oldid + ".agv"), Path.Combine(EditGame.SrcResDir, EditGame.Views[NewResNum].ID + ".agv"), true);
                    }
                }
            }
        }
        ...
        public void EditViewProperties(int FirstProp) {
            ...
            if (GetNewResID(AGIResType.View, ViewNumber, ref id, ref description, InGame, FirstProp)) {
                ...
                MDIMain.RefreshPropertyGrid(AGIResType.View, ViewNumber);
            }
        }
        ...
        private void SetEScrollbars(float oldscale = 0) {
            ...
            bool showHSB = picCel.Width > splitCanvas.Panel1.ClientSize.Width - 2 * VE_MARGIN;
            bool showVSB = picCel.Height > splitCanvas.Panel1.ClientSize.Height - 2 * VE_MARGIN - (showHSB ? hsbCel.Height : 0);
        ...
        internal void ChangeHeight(int newHeight, bool DontUndo = false) {
            ...
            if (!DontUndo) {
                ...
                if (newHeight > height) {
                    ...
                    byte[,] tmpCelData = new byte[CelWidth, height - newHeight];
                    for (int j = newHeight; j < height; j++) {
                        for (int i = 0; i < CelWidth; i++) {
                            // add this cel pixel's color
                            tmpCelData[i, j - newHeight] = EditView[SelectedLoop][SelectedCel][i, j];
                        }
                    }
            ...
            // set new height
            EditView[SelectedLoop][SelectedCel].Height = (byte)newHeight;
            // redraw cel
            DisplayCel();
            if (ShowPreview) {
                DisplayPrevLoop();
        ...
        internal void ChangeWidth(int newWidth, bool DontUndo = false) {
            ...
            if (!DontUndo) {
                ...
                if (newWidth > width) {
                    ...
                    // store data being eliminated
                    byte[,] tmpCelData = new byte[width - newWidth, height];
                    for (int i = newWidth; i < width; i++) {
                        for (int j = 0; j < height; j++) {
                            // add this cel pixel's color
                            tmpCelData[i - newWidth, j] = EditView[SelectedLoop][SelectedCel][i, j];
                        }
                    }
                    NextUndo.CelData = tmpCelData;
                }
            ...
            // set new width
            EditView[SelectedLoop][SelectedCel].Width = (byte)newWidth;
            // redraw cel
            DisplayCel();
            if (ShowPreview) {
                DisplayPrevLoop();
        ...
        internal void ChangeCelSize(int newWidth, int newHeight, bool DontUndo = false) {
            int width = EditView[SelectedLoop][SelectedCel].Width;
            int height = EditView[SelectedLoop][SelectedCel].Height;
            ...
            if (newWidth != width && newHeight == height) {
                // width only
                ChangeWidth(newWidth, DontUndo);
                return;
            }
            if (newWidth == width && newHeight != height) {
                // height only
                ChangeHeight(newHeight, DontUndo);
                return;
            }
            if (!DontUndo) {
                ViewUndo NextUndo = new() {
                    LoopNumber = SelectedLoop,
                    CelNumber = SelectedCel,
                    UndoData = [width, height],
                    Action = ChangeSize
                };
                // if new size is smaller than old size (either direction)
                if (newWidth < width || newHeight < height) {
                    // store data being eliminated
                    NextUndo.CelData = (byte[,])EditView[SelectedLoop][SelectedCel].AllCelData.Clone();
                }
                AddUndo(NextUndo);
            }
            // set new size
        ...
        private void BeginMoveSelection(Point anchorpt) {
            ...
            CurrentOperation = ViewEditOperation.MoveSelection;
        ...
        private void SetPScrollbars(float oldscale = 0) {
            ...
            // determine if scrollbars are necessary
            bool showHSB = pnlPreview.Width > (splitCanvas.Panel2.ClientSize.Width - 2 * VE_MARGIN);
            // don't forget to allow for the toolstrip height
            bool showVSB = pnlPreview.Height > (splitCanvas.Panel2.ClientSize.Height - 2 * VE_MARGIN - 2 * toolStrip2.Height);
            // check horizontal again(in case addition of vert scrollbar forces it to be shown)
        ...
        private byte NearestAGIColor(Color sourcecolor) {
            ...
            for (byte i = 0; i < 16; i++) {
                ...
                newdifference = Math.Abs(red - EditPalette[i].R) +
                                Math.Abs(green - EditPalette[i].G) +
                                Math.Abs(blue - EditPalette[i].B);
        ...
        internal void ShowHelp() {
            string topic = "htm\\winagi\\editor_view.htm";
        ...
        public class ViewEditCelProperties {
            ...
            public int Width {
                get {
                    if (parent.CurrentOperation == ViewEditOperation.ChangeBoth ||
                       parent.CurrentOperation == ViewEditOperation.ChangeWidth) {
            ...
            public int Height {
                get {
                    if (parent.CurrentOperation == ViewEditOperation.ChangeBoth ||
                        parent.CurrentOperation == ViewEditOperation.ChangeHeight) {
