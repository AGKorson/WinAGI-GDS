...
namespace WinAGI.Engine {
    ...
    public class Sound : AGIResource {
        ...
        internal (ResourceErrorType, int) LoadTracks() {
            int length = 0, tracklength;
            int startPos, duration = 0;
            short freqdivisor;
            byte attenuation;
                    ...
                    while (Pos < Size - 1) {
                        ...
                        if (i < 3) {
                            ...
                            if (duration > 0) {
                                if (Pos + 2 < Size) {
                                    // frequency
                                    freqdivisor = (short)(16 * (mData[Pos] & 0x3F) + (mData[Pos + 1] & 0xF));
                                    // attenuation
                                    attenuation = ((byte)(mData[Pos + 2] & 0xF));
                                    mTrack[i].Notes.Add(freqdivisor, duration, attenuation);
                            ...
                            if (duration > 0) {
                                if (Pos + 2 < Size) {
                                    ...
                                    freqdivisor = ((short)(mData[Pos + 1] & 7));
                                    // attenuation
                                    attenuation = (byte)(mData[Pos + 2] & 0xF);
                                    mTrack[3].Notes.Add(freqdivisor, duration, attenuation);
                                    tracklength += duration;
                                }
                                else {
                                    // zero length note
        ...
        public void PlaySound(SoundPlaybackMode playmode = SoundPlaybackMode.PCSpeaker) {
            ...
            // for now, using PCJr mode with only channel 1 enabled is good enough
            WinAGIException.ThrowIfNotLoaded(this);
            switch (mFormat) {
            case SoundFormat.AGI:
                // use passed mode
        ...
        public void SaveProps() {
            if (mInGame) {
                ...
                parent.WriteGameSetting(section, "ID", mResID, "Sounds");
                ...
                parent.WriteGameSetting(section, "Mute0", mTrack[0].Muted);
                parent.WriteGameSetting(section, "Mute1", mTrack[1].Muted);
                parent.WriteGameSetting(section, "Mute2", mTrack[2].Muted);
                parent.WriteGameSetting(section, "Mute3", mTrack[3].Muted);
                parent.WriteGameSetting(section, "Visible0", mTrack[0].Visible);
                parent.WriteGameSetting(section, "Visible1", mTrack[1].Visible);
                parent.WriteGameSetting(section, "Visible2", mTrack[2].Visible);
                parent.WriteGameSetting(section, "Visible3", mTrack[3].Visible);
