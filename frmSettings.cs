...
namespace WinAGI.Editor {
    public partial class frmSettings : Form {
        ...
        private void btnDefault_Click(object sender, EventArgs e) {
            ...
            NewSettings.ShowInfoGrid.Reset();
        ...
        private void btnOK_Click(object sender, EventArgs e) {
            ...
            bool changeNotes = (NewSettings.ShowNotes.Value != WinAGISettings.ShowNotes.Value) || (WinAGISettings.OneTrack.Value != NewSettings.OneTrack.Value);
            bool changeResName = (NewSettings.IncludeResNum.Value != WinAGISettings.IncludeResNum.Value && !NewSettings.ShowResNum.Value) ||
            ...
            bool changeShowPics = NewSettings.LEShowPics.Value != WinAGISettings.LEShowPics.Value;
            ...
            ResListType listtype = WinAGISettings.ResListType.Value;
            ...
            if (EditGame is not null) {
                ...
                default:
                    ...
                    else if (WinAGISettings.ResListType.Value != listtype) {
                        SelResType = AGIResType.Game;
                        SelResNum = -1;
                        MDIMain.BuildResourceTree();
                        MDIMain.ShowResTree();
                ...
                // set preview font
                PreviewWin.rtfLogPrev.Font = new Font(WinAGISettings.PreviewFontName.Value, WinAGISettings.PreviewFontSize.Value);
                ...
                if (WinAGISettings.ShowPreview.Value) {
                    ...
                    if (!PreviewWin.Visible) {
                        ...
                        if (SelResType >= AGIResType.Logic && SelResType <= AGIResType.View && SelResNum >= 0) {
                ...
                if (changeResName || WinAGISettings.ResListType.Value != ResListType.None) {
                    switch (WinAGISettings.ResListType.Value) {
                    case ResListType.TreeList:
                        // step through all resources, and reassign tree list caption
                        foreach (TreeNode node in HdrNode[0].Nodes) {
                            node.Text = ResourceName(EditGame.Logics[(int)node.Tag], true);
                        }
                        foreach (TreeNode node in HdrNode[1].Nodes) {
                            node.Text = ResourceName(EditGame.Pictures[(int)node.Tag], true);
                        }
                        foreach (TreeNode node in HdrNode[2].Nodes) {
                            node.Text = ResourceName(EditGame.Sounds[(int)node.Tag], true);
                        }
                        foreach (TreeNode node in HdrNode[3].Nodes) {
                            node.Text = ResourceName(EditGame.Views[(int)node.Tag], true);
                        }
                        break;
                    case ResListType.ComboList:
                        ...
                        case 1:
                            foreach (ListViewItem listitem in MDIMain.lstResources.Items) {
                                listitem.Text = ResourceName(EditGame.Logics[(int)listitem.Tag], true);
                            }
                            break;
                        case 2:
                            foreach (ListViewItem listitem in MDIMain.lstResources.Items) {
                                listitem.Text = ResourceName(EditGame.Pictures[(int)listitem.Tag], true);
                            }
                            break;
                        case 3:
                            foreach (ListViewItem listitem in MDIMain.lstResources.Items) {
                                listitem.Text = ResourceName(EditGame.Sounds[(int)listitem.Tag], true);
                            }
                            break;
                        case 4:
                            foreach (ListViewItem listitem in MDIMain.lstResources.Items) {
                    ...
                    foreach (frmLogicEdit frm in LogicEditors) {
                        if (frm.FormMode == LogicFormMode.Logic) {
                            if (frm.InGame) {
                                frm.Text = LOGIC_EDITOR + ResourceName(frm.EditLogic, true, true);
                                if (frm.IsChanged) {
                    ...
                    foreach (frmPicEdit frm in PictureEditors) {
                        if (frm.InGame) {
                            frm.Text = PICTURE_EDITOR + ResourceName(frm.EditPicture, true, true);
                            if (frm.IsChanged) {
                                frm.Text = CHG_MARKER + frm.Text;
                            }
                        }
                    ...
                    foreach (frmSoundEdit frm in SoundEditors) {
                        if (frm.InGame) {
                            frm.Text = SOUND_EDITOR + ResourceName(frm.EditSound, true, true);
                            if (frm.IsChanged) {
                                frm.Text = CHG_MARKER + frm.Text;
                            }
                        }
                    ...
                    foreach (frmViewEdit frm in ViewEditors) {
                        if (frm.InGame) {
                            frm.Text = VIEW_EDITOR + ResourceName(frm.EditView, true, true);
                            if (frm.IsChanged) {
                                frm.Text = CHG_MARKER + frm.Text;
                            }
                        }
            ...
            LogicDecoder.DefaultSrcExt = WinAGISettings.DefaultExt.Value.ToLower();
            Engine.Base.ErrorLevel = WinAGISettings.ErrorLevel.Value;
            ...
            // RESET WARNINGS
            if (chkResetWarnings.Checked || resetWarnings) {
                ...
                WinAGISettings.NotifyLoadWarn.Reset(WinAGISettingsFile);
                ...
                WinAGISettings.Warn2089Mirror.Reset(WinAGISettingsFile);
                for (int i = 0; i < Engine.Base.NoCompWarn.Length; i++) {
                    Engine.Base.NoCompWarn[i] = false;
                }
                for (int i = 0; i <= 4; i++) {
                    //  (int)((WARNCOUNT - 1) / 30)
        ...
        private void chkShowInfoGrid_CheckedChanged(object sender, EventArgs e) {
            NewSettings.ShowInfoGrid.Value = chkShowInfoGrid.Checked;
        }
        ...
        private void cmdColor_Click(object sender, EventArgs e) {
            ...
            if (index < 10) {
                cdColors.Color = NewSettings.SyntaxStyle[index].Color.Value;
            }
            else {
            ...
            if (cdColors.ShowDialog(MDIMain) == DialogResult.OK) {
                if (index < 10) {
                    NewSettings.SyntaxStyle[index].Color.Value = cdColors.Color;
                }
                else {
        ...
        private void lstColors_SelectedIndexChanged(object sender, EventArgs e) {
            ...
            if (index < 10) {
                picColor.BackColor = NewSettings.SyntaxStyle[index].Color.Value;
            }
            else {
            ...
            if (index == 0 || (index < 10 && lstColors.Items.Count > 2)) {
                chkBold.Enabled = true;
                chkItalic.Enabled = true;
                chkBold.Checked = (NewSettings.SyntaxStyle[index].FontStyle.Value & FontStyle.Bold) == FontStyle.Bold;
                chkItalic.Checked = (NewSettings.SyntaxStyle[index].FontStyle.Value & FontStyle.Italic) == FontStyle.Italic;
            }
            else {
        ...
        private void cmdInstReset_Click(object sender, EventArgs e) {
            ...
            foreach (Sound sound in EditGame.Sounds) {
                ...
                for (int i = 0; i < 3; i++) {
                    sound[0].Instrument = NewSettings.DefInst[0].Value;
                    sound[0].Muted = NewSettings.DefMute[0].Value;
                }
                sound.Save();
                if (!loaded) {
                    sound.Unload();
        ...
        private void udSoundScale_SelectedItemChanged(object sender, EventArgs e) {
            NewSettings.SndZoom.Value = int.Parse(udSoundScale.Text);
        }
        #endregion
        ...
        private void lstLEColors_SelectedIndexChanged(object sender, EventArgs e) {
            ...
            case 0:
                color = NewSettings.RoomEdgeColor.Value;
                break;
            case 1:
                color = NewSettings.RoomFillColor.Value;
                break;
            case 2:
                color = NewSettings.TransPtEdgeColor.Value;
                break;
            case 3:
                color = NewSettings.TransPtFillColor.Value;
                break;
            case 4:
                color = NewSettings.CmtEdgeColor.Value;
                break;
            case 5:
                color = NewSettings.CmtFillColor.Value;
                break;
            case 6:
                color = NewSettings.ErrPtEdgeColor.Value;
                break;
            case 7:
                color = NewSettings.ErrPtFillColor.Value;
                break;
            case 8:
                color = NewSettings.ExitEdgeColor.Value;
                break;
            case 9:
                color = NewSettings.ExitOtherColor.Value;
                break;
            }
            picLEColor.BackColor = color;
        }

        private void lstLEColors_DoubleClick(object sender, EventArgs e) {
        ...
        private void udLayoutScale_SelectedItemChanged(object sender, EventArgs e) {
            NewSettings.LEScale.Value = int.Parse(udLayoutScale.Text);
        }
        ...
        private void InitForm() {
            ...
            chkIncludeResNum.Enabled = !NewSettings.ShowResNum.Value;
            int index = NewSettings.ResFormatNameCase.Value * 4;
            ...
            cmbResFormat.SelectedIndex = index;
            cmbResFormat.Enabled = NewSettings.ShowResNum.Value;
            ...
            chkHidePreview.Enabled = NewSettings.ShowPreview.Value;
            chkShowInfoGrid.Checked = NewSettings.ShowInfoGrid.Value;
            ...
            udSoundScale.Text = NewSettings.SndZoom.Value.ToString();
            ...
            lstLEColors.SelectedIndex = 0;
            udLayoutScale.Text = NewSettings.LEScale.Value.ToString();
            DrawLESample();
        }
        ...
        private void SaveNewSettings() {
            ...
            WinAGISettings.ShowInfoGrid.WriteSetting(WinAGISettingsFile);
