...
namespace WinAGI.Editor {
    public partial class frmPicPrintPrev : Form {
        ...
        private void cmdOK_Click(object sender, EventArgs e) {
            ...
            // all background colors available only if PowerPack is active
            if (EditGame is not null && EditGame.PowerPack) {
                PTInfo.BGColor = cmbBG.SelectedIndex;
            }
            else {
                PTInfo.BGColor = cmbBG.SelectedIndex == 0 ? 0 : 15;
            }
            if (optPrint.Checked) {
        ...
        private void cmdCopy_Click(object sender, EventArgs e) {
            ...
            string output = rtfLine1.Text + Environment.NewLine;
            if (rtfLine2.Visible) {
                output += rtfLine2.Text + Environment.NewLine;
            }
            Clipboard.Clear();
        ...
        private void lblScreenSize_DoubleClick(object sender, EventArgs e) {
            // toggle screen size
            if (PTInfo.MaxCol == 39) {
                PTInfo.MaxCol = 79;
                PTInfo.CharWidth = 4;
                lblScreenSize.Text = "Screen Size: 80 x 25";
            }
            else {
                PTInfo.MaxCol = 39;
                PTInfo.CharWidth = 8;
                lblScreenSize.Text = "Screen Size: 40 x 25";
            }
        }
        ...
        private void UpdateCommand() {
            ...
            if (optPrint.Checked) {
                ...
                Define[] reservedcolor;
                if (EditGame is null) {
                    reservedcolor = Engine.Base.DefaultReservedDefines.ColorNames;
                }
                else {
                    reservedcolor = EditGame.ReservedDefines.ColorNames;
                }
                if (EditGame is not null && EditGame.PowerPack) {
