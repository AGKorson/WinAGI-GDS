    ...
    public class WAVPlayer {
        ...
        internal byte[] BuildPCjrWAV(Sound sound) {
            ...
            if (sound.parent is null) {
                // default to V2 if no parent (maybe later add a property
                // that allows overriding this for sounds without a parent?)
                dissolveData = dissolveDataV2;
            }
            else {
                dissolveData = sound.parent.agIntVersion.IsV3 ? dissolveDataV3 : dissolveDataV2;
            }
        ...
        public sealed class SN76496 {
            ...
            private void UpdateToneChannel(int channel) {
                ...
                if (channelCounterReload[channel] == 0)
                    return;
