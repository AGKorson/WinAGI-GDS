...
namespace WinAGI.Editor {
    public class Exits : IEnumerable<Exit> {
        ...
        public void CopyFrom(Exit[] exits) {
        ...
        public void CopyFrom(Exits exits, bool locations = false) {
            // first clear existing collection
            Clear();
            // then add the exits from the array
            foreach (Exit exit in exits) {
                // ID is a string, we need the number
                Exit newexit = Add(exit.ID[2..].IntVal(), exit.Room, exit.Reason, exit.Style, exit.Type, exit.TypeIndex, exit.Leg);
                if (locations) {
                    newexit.EP = exit.EP;
                    newexit.SP = exit.SP;
                }
