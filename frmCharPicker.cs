...
namespace WinAGI.Editor {
    public partial class frmCharPicker : Form {
        ...
        private void frmCharPicker_HelpRequested(object sender, HelpEventArgs hlpevent) {
            Help.ShowHelp(Base.HelpParent, Base.WinAGIHelp, "htm\\winagi\\charmap.htm");
        ...
        private void picSelect_MouseClick(object sender, MouseEventArgs e) {
            ...
            CurX = Math.Clamp(CurX, 0, 15);
            CurY = Math.Clamp(CurY, 0, 15);
            if (CurX == 0 && CurY == 0) {
                return;
            }
        ...
        private void picSelect_MouseDoubleClick(object sender, MouseEventArgs e) {
            ...
            int charval = CurX + CurY * 16;
            switch (charval) {
            case 0:
                // nothing to add
                return;
            default:
                SelChar.Value = (byte)charval;
                switch (charval) {
                case 10:
                ...
                case 255:
                    ...
                    if (charval / 16 > 9) {
                        AddChar(charval / 16 + 55);
                    }
                    else {
                        AddChar(charval / 16 + 48);
                    }
                    if (charval % 16 > 9) {
                        AddChar(charval % 16 + 55);
                    }
        ...
        private void picInsert_MouseDown(object sender, MouseEventArgs e) {
            ...
            if (CurX < 0)
                CurX = 0;
            if (CurX > InsertString.Length)
                CurX = InsertString.Length;
        ...
        private void picInsert_Paint(object sender, PaintEventArgs e) {
            ...
            for (int i = 0; i < InsertString.Length; i++) {
                charval = new(InsertString[i]);
                Rectangle charsource = new(charval.X * 16, charval.Y * 16, 16, 16);
                if (sellength > 0 && i >= SelStart && i < SelEnd) {
        ...
        private void AddChar(int charval) {
            AddChar((char)charval);
        }
        ...
        private void AddChar(char charval) {
            if (SelStart != SelEnd) {
                // replace the selection with the new char and reset cursor
                InsertString = InsertString.Left(SelStart) + charval.ToString() + InsertString.Right(InsertString.Length - SelEnd);
                CursorPos = SelStart + 1;
                SelStart = SelEnd = CursorPos;
            }
            else {
                // insert char at cursor position
                InsertString = InsertString.Left(CursorPos) + charval.ToString() + InsertString.Right(InsertString.Length - CursorPos);
                SelStart = SelEnd = ++CursorPos;
            }
            btnInsert.Enabled = InsertString.Length > 0;
        }
        ...
        private void ShowToolTip() {
            ...
            if (mp.Y < 0)
                return;
            if (mp.X >= picSelect.Width)
                return;
            if (mp.Y >= picSelect.Height)
                return;
