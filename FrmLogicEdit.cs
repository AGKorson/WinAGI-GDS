...
namespace WinAGI.Editor {
    public partial class frmLogicEdit : Form {
        ...
        private void fctb_TextChanged(object sender, TextChangedEventArgs e) {
            ...
            if (FormMode == LogicFormMode.Logic && InGame && !saving) {
                ...
                if (added != 0 || deleted != 0) {
                    if (added > 0) {
                        UpdateWarningList(LogicNumber, chgStart, added, 1);
                    }
                    else if (deleted > 0) {
                        if (deleted == 1 && chgLength <= 1) {
                            UpdateWarningList(LogicNumber, chgStart, 1, 3);
                        }
                        else {
                            UpdateWarningList(LogicNumber, chgStart, deleted, 2);
                        }
                    }
                    // refresh counts
                ...
                if (msgtoken.Type == AGITokenType.Identifier) {
                    ...
                    if (argnum >= 0) {
                        ...
                        if (argnum == -1 && msgtoken.Text.Length > 0 && msgtoken.Text[0] == 's') {
                            ...
                            int num;
                            if (int.TryParse(valuetext, out num)) {
                                // possible string assignment
        ...
        private static void RegisterStyles(FastColoredTextBox tb) {
            ...
            // Register styles in the exact order you want them layered.
        ...
        internal void SaveLogicSource() {
            ...
            if (InGame) {
                ...
                UpdateAutoIncludes();
                // instead of using logic source to update exits, use the fctb?
                //
                ...
                // TODO: change update to modify editor, instead of replacing entire text
                ...
                case ArgListType.View:
                    for (int j = 0; j < 4; j++) {
                        if (ArgType == ArgListType.All ||
                            ...
                            (int)(ArgType - ArgListType.Logic) == j) {
                            for (int i = 0; i < 256; i++) {
        ...
        private void ShowDefineList() {
            ...
            if (fctb.SelectionLength > 0) {
                ...
                if (!selected) {
                    bool found = false;
                    foreach (ListViewItem tmpItem in lstDefines.Items) {
                        switch (seltext.ToUpper().CompareTo(tmpItem.Text.ToUpper())) {
                        case 0:
                            // select this one
                            tmpItem.Selected = true;
        ...
        private void ShowCommandList() {
            ...
            if (fctb.SelectionLength > 0) {
                ...
                if (!selected) {
                    bool found = false;
                    foreach (ListViewItem tmpItem in lstDefines.Items) {
                        switch (seltext.ToLower().CompareTo(tmpItem.Text.ToLower())) {
                        case 0:
                            // select this one
                            tmpItem.Selected = true;
                            lstDefines.TopItem = tmpItem;
                ...
                case DialogResult.No:
                    if (InGame) {
                        if (FormMode == LogicFormMode.Logic) {
                            ...
                            // update and save warning/error info for this logic
                            MDIMain.ResetWarningList((byte)LogicNumber, WarningList);
                        }
                        else {
                            //
                        }
