...
namespace WinAGI.Editor {
    public partial class frmLayout : Form {
        ...
        private const float ERRPT_SIZE = 0.6f;
        ...
        private void frmLayout_HelpRequested(object sender, HelpEventArgs hlpevent) {
            ShowHelp();
        ...
        private void mnuHideAllPics_Click(object sender, EventArgs e) {
            WinAGISettings.LEShowPics.Value = false;
            for (int i = 0; i < 256; i++) {
        ...
        private void mnuShowAllPics_Click(object sender, EventArgs e) {
            WinAGISettings.LEShowPics.Value = true;
            for (int i = 0; i < 256; i++) {
                Room[i].ShowPic = true;
            }
            DrawLayout();
        ...
        private void SetEditMenu() {
            ...
            mnuDelete.Enabled = Selection.Type != LayoutSelection.None &&
                Selection.Type != LayoutSelection.Multiple;
            ...
            //    mnuInsertTransfer
            //    mnuInsertComment
            // insertRoom only allowed if select tool is active
            ...
            // insertComment only allowed if select tool is active
            ...
            else if (Selection.Type == LayoutSelection.Exit && Selection.Direction == ExitDirection.OneWay) {
                mnuToggleTransPt.Visible = true;
        ...
        private void mnuHideRoom_Click(object sender, EventArgs e) {
            // hide this room
            if (Selection.Type != LayoutSelection.Room ||
                EditGame.Logics[Selection.Number].Number <= 0) {
                if (Selection.Type == LayoutSelection.Room) {
                    Debug.Assert(EditGame.Logics[Selection.Number].Number > 0);
                }
                return;
            }
            // update logic
        ...
        private void mnuToggleTransPt_Click(object sender, EventArgs e) {
            ...
            case LayoutSelection.Exit:
                // switch to other leg
                if (Selection.Direction == ExitDirection.OneWay) {
        ...
        private void mnuProperties_Click(object sender, EventArgs e) {
            ...
            string id = EditGame.Logics[Selection.Number].ID;
            string description = EditGame.Logics[Selection.Number].Description;

            // use the id/description change method
            if (GetNewResID(AGIResType.Logic, Selection.Number, ref id, ref description, true, 1)) {
        ...
        private void tmrScroll_Tick(object sender, EventArgs e) {
            ...
            PointF mp = picDraw.PointToClient(MousePosition);
            if (mp.X < -10) {
                Offset.X += delta.X;
            ...
            case LayoutTool.Select:
                if (MoveExit) {
                    ...
                    case LayoutCursor.OtherExit:
                        ...
                        if (Selection.Direction != ExitDirection.BothWays) {
                            // if changing to room
                            if (MovePoint == 1) {
                            ...
                            tmpSel.Direction = ExitDirection.BothWays;
                            SelectExit(tmpSel);
                            // redraw
                            DrawLayout();
                            MarkAsChanged();
            ...
            if (MoveExit || DrawExit != 0) {
                ...
                case LayoutSelection.ErrPt:
                    PointF[] errPoints = [new PointF(mp.X, mp.Y),
                        new PointF(mp.X + ERRPT_SIZE * DSF, mp.Y),
                        new PointF(mp.X + ERRPT_SIZE / 2 * DSF, mp.Y + 0.866f * ERRPT_SIZE * DSF)
                    ];
            ...
            case Keys.Shift:
                ...
                case Keys.Up or Keys.Down or Keys.Left or Keys.Right:
                    if (WinAGISettings.LEUseGrid.Value) {
                        ...
                        // default is one room width
                        sngOffset = RM_SIZE * DSF;
                    }
        ...
        private void hScrollBar1_MouseDown(object sender, MouseEventArgs e) {
            ...
            // left arrow
            Rectangle leftArrow = new(0, 0, arrowSize, sb.Height);
            // right arrow
            Rectangle rightArrow = new(sb.Width - arrowSize, 0, arrowSize, sb.Height);
        ...
        private void vScrollBar1_MouseDown(object sender, MouseEventArgs e) {
            ...
            // Up arrow
            Rectangle upArrow = new(0, 0, sb.Width, arrowSize);
            // Down arrow
            Rectangle downArrow = new(0, sb.Height - arrowSize, sb.Width, arrowSize);
        ...
        internal bool GetLayoutData() {
            ...
            string fileName = Path.Combine(EditGame.GameDir, EditGame.GameID + ".wal");
            if (File.Exists(fileName)) {
                ...
                if (File.Exists(fileName)) {
                    ...
                    // rename the file
                    try {
                        File.Move(fileName, Path.Combine(EditGame.GameDir, EditGame.GameID + ".wal"));
                    }
                    catch {
                    ...
                    fileName = Path.Combine(EditGame.GameDir, EditGame.GameID + ".wal");
                }
                else {
                    filefound = false;
                }
            ...
            if (filefound) {
                if (IsV3Layout()) {
                    return GetV3LayoutData(fileName);
                }
                else {
                    return GetV2LayoutData(fileName);
                }
            }
            else {
                // notify user
                if (MDIMain.MsgBoxWithHelp(
                    ...
                    MessageBoxButtons.YesNo,
                    MessageBoxIcon.Question,
                    "htm\\winagi\\editor_layout.htm") == DialogResult.Yes) {
                    if (ExtractLayout()) {
        ...
        private static bool IsV3Layout() {
            // return true if the layout file is a json file
            try {
                using StreamReader sr = File.OpenText(Path.Combine(EditGame.GameDir, EditGame.GameID + ".wal"));
                int next = sr.Read();
                while (next != -1) {
                    switch (next) {
                    case '{':
        ...
        private bool GetV3LayoutData(string layoutfile) {
            ...
            try {
                ...
                // read header block
                LayoutFileHeader layoutdata = JsonSerializer.Deserialize<LayoutFileHeader>(line);
                // check version
                ...
                while ((line = sr.ReadLine()) is not null) {
                    ...
                    try {
                        layoutobj = JsonSerializer.Deserialize<LayoutFileData>(line);
                    }
                    catch (JsonException) {
                    ...
                    if (layoutobj is LFDRoom room) {
                        ...
                        if (Room[renum.OldNumber].Visible) {
                        ...
                        Room[showroom.Index].ShowPic = WinAGISettings.LEShowPics.Value;
                        ...
                        if (needpos) {
                            Room[showroom.Index].Loc = GetInsertPos(
                                ...
                                (picDraw.ClientSize.Height / 2 - Offset.Y) / DSF);
            ...
            if (warning) {
                ...
                try {
                    ...
                    string linetext = "Errors were encountered in the layout file. WinAGI " +
            ...
            void LayoutUpdate(int index, Exit[] exits) {
        ...
        private bool GetV2LayoutData(string layoutfile) {
            ...
            for (linenum = 0; linenum < layoutData.Count; linenum++) {
                linetext = layoutData[linenum].Trim();
                ...
                case 1:
                    // Line 2: ObjCount
                    if (linetext.IsInt()) {
                        fileCount = objCount = int.Parse(linetext);
                    }
                    else {
                        // invalid data
                        return ExtractIfError();
            ...
            for (linenum++; linenum < layoutData.Count; linenum++) {
                linetext = layoutData[linenum].Trim();
                // skip blank lines
                if (linetext.Length == 0) {
                    continue;
                ...
                switch (strData[0].Length != 0 ? strData[0][0] : 0) {
                case 67:
                    // C:  comment should have exactly 9 elements
                    if (strData.Length != 9) {
                        // log a warning
                        warning = true;
                        errorList.Add("Line " + (linenum + 1).ToString() + ": invalid COMMENT entry");
                        // try next line
                        continue;
                    }
                    break;
                case 69:
                    ...
                    if (strData.Length != 9) {
                        // log a warning
                        warning = true;
                        errorList.Add("Line " + (linenum + 1).ToString() + ": invalid ERRPT entry");
                        // try next line
                        continue;
                    }
                    break;
                case 78:
                    ...
                    if (strData.Length != 3) {
                        // log a warning
                        warning = true;
                        errorList.Add("Line " + (linenum + 1).ToString() + ": invalid RENUMBER entry");
                        // try next line
                        continue;
                    }
                    break;
                case 82:
                    ...
                    if (strData.Length < 7) {
                        // log a warning
                        warning = true;
                        errorList.Add("Line " + (linenum + 1).ToString() + ": invalid ROOM entry");
                        // try next line
                        continue;
                    }
                    break;
                case 84:
                    ...
                    if (strData.Length != 13) {
                        // log a warning
                        warning = true;
                        errorList.Add("Line " + (linenum + 1).ToString() + ": invalid TRANSPT entry");
                        // try next line
                        continue;
                    }
                    break;
                case 85:
                    ...
                    if (strData.Length < 3) {
                        // log a warning
                        warning = true;
                        errorList.Add("Line " + (linenum + 1).ToString() + ": invalid UPDATE entry");
                        // try next line
                        continue;
                    }
                    break;
                default:
                    ...
                    warning = true;
                    errorList.Add("Line " + (linenum + 1) + ": line does not contain valid line marker");
                    // try next line
                    continue;
                }
                // get number
                ...
                if (number == 0) {
                    warning = true;
                    errorList.Add("Line " + (linenum + 1).ToString() + ": invalid object number ( < 1)");
                    // try next line
                    continue;
                }
                ...
                if (number > 255) {
                    warning = true;
                    errorList.Add("Line " + (linenum + 1).ToString() + ": invalid object number ( > 255)");
                    // try next line
                    continue;
                }
                ...
                if (strData[0][0] != 'N') {
                    ...
                    if (bool.TryParse(strData[2], out tmpVisible) == false) {
                        ...
                        warning = true;
                        errorList.Add("Line " + (linenum + 1).ToString() + ": invalid visible property");
                        // try next line
                        continue;
                    }
                }
                ...
                case 67 or 69 or 82 or 84:
                    ...
                    if (tmpOrder >= objCount) {
                        ...
                        // warn user
                        warning = true;
                        errorList.Add("Line " + (linenum + 1).ToString() + ": invalid object position number");
                    }
                ...
                case 67:
                    ...
                    Comment[number].Visible = tmpVisible;
                    // set obj order
                    objOrder[tmpOrder].Type = LayoutSelection.Comment;
                    objOrder[tmpOrder].Number = number;
                    Comment[number].Order = tmpOrder;
                    // set coordinates
                    Comment[number].Loc = new((float)strData[4].Val(), (float)strData[5].Val());
                    // set height/width
                    Comment[number].Size = new((float)strData[6].Val(), (float)strData[7].Val());
                    Comment[number].Text = strData[8].Replace("&crlf", "\r\n");
                    count++;
                    break;
                case 69:
                    ...
                    objOrder[tmpOrder].Number = number;
                    // set visibility property
                    ErrPt[number].Visible = tmpVisible;
                    ErrPt[number].Order = tmpOrder;
                    ErrPt[number].Loc = new((float)strData[4].Val(), (float)strData[5].Val());
                    ErrPt[number].Room = strData[6].IntVal();
                    ErrPt[number].ExitID = strData[7];
                    ErrPt[number].FromRoom = strData[8].IntVal();
                    count++;
                    break;
                case 78:
                    // "N" - renumbering 'N |##|##|--
                    // validate second number
                    int renumber = strData[2].IntVal();
                    if (renumber < 1 || renumber > 255) {
                        warning = true;
                        errorList.Add("Line " + (linenum + 1).ToString() + ": a Renumber action has invalid object numbers");
                        break; // skip to next line
                    }
                    // update objOrder entry to point to newroom
                    objOrder[Room[number].Order].Number = renumber;
                    // move oldroom data to newroom
                    Room[renumber].Loc = Room[number].Loc;
                    Room[renumber].ShowPic = Room[number].ShowPic;
                    Room[renumber].Visible = Room[number].Visible;
                    Room[renumber].Exits.CopyFrom(Room[number].Exits);
                    Room[renumber].Order = Room[number].Order;
                    // clear old room
                    Room[number].Loc = new();
                    Room[number].ShowPic = false;
                    Room[number].Visible = false;
                    Room[number].Exits.Clear();
                    Room[number].Order = 0;
                    for (int i = 1; i < 256; i++) {
                        // update room exits
                        for (int j = 0; j < Room[i].Exits.Count; j++) {
                            if (Room[i].Exits[j].Room == number) {
                                Room[i].Exits[j].Room = renumber;
                            }
                        }
                        // update trans pts
                        if (TransPt[i].Count != 0) {
                            for (int j = 0; j < 2; j++) {
                                if (TransPt[i].Room[j] == number) {
                                    TransPt[i].Room[j] = (byte)renumber;
                                }
                            }
                        }
                        // update errpts
                        if (ErrPt[i].Visible) {
                            if (ErrPt[i].FromRoom == number) {
                                ErrPt[i].FromRoom = renumber;
                            }
                        }
                    }
                    // mark as changed so it gets updated
                ...
                case 82:
                    ...
                    objOrder[tmpOrder].Number = number;
                    Room[number].Order = tmpOrder;
                    Room[number].Visible = tmpVisible;

                    // get pic status and coords
                    bool tmpShowPic = true;
                    _ = bool.TryParse(strData[4], out tmpShowPic);
                    Room[number].ShowPic = tmpShowPic;
                    Room[number].Loc = new((float)strData[5].Val(), (float)strData[6].Val());
                    // get exit info
                    Room[number].Exits = ParseExits(strData);
                    count++;
                    break;
                case 84:
                    ...
                    objOrder[tmpOrder].Number = number;
                    TransPt[number].Order = tmpOrder;
                    // set first transfer coordinates
                    TransPt[number].Loc[0] = new((float)strData[4].Val(), (float)strData[5].Val());
                    // get next pair of coordinates
                    TransPt[number].Loc[1] = new((float)strData[6].Val(), (float)strData[7].Val());
                    // get rooms
                    TransPt[number].Room[0] = (byte)strData[8].IntVal();
                    TransPt[number].Room[1] = (byte)strData[9].IntVal();
                    // get exits
                    TransPt[number].ExitID[0] = strData[10];
                    TransPt[number].ExitID[1] = strData[11];
                    // get Count Value
                    TransPt[number].Count = strData[12].IntVal();
                    count++;
                    break;
                case 85:
                    // "U" - updated room info
                    ...
                    for (int i = 3; i < strData.Length; i++) {
                        ...
                        if (strExit.Length != 4) {
                            warning = true;
                            errorList.Add("Line " + (linenum + 1).ToString() + ": Update line has invalid data");
                        }
                        else {
                    ...
                    if (tmpVisible) {
                        ...
                        // add a new room
                        bool needpos = !Room[number].Visible;
                        Room[number].Visible = true;
                        Room[number].ShowPic = WinAGISettings.LEShowPics.Value;
                        // the update exits don't include transfer/errpt info- check
                        // them against current exits
                        foreach (var exit in tmpExits) {
                            for (int i = 0; i < Room[number].Exits.Count; i++) {
                                // if exit already exists, update it
                                if (Room[number].Exits[i].Room == exit.Room &&
                                    Room[number].Exits[i].Reason == exit.Reason &&
                                    Room[number].Exits[i].Style == exit.Style) {
                                    exit.Type = Room[number].Exits[i].Type;
                                    exit.TypeIndex = Room[number].Exits[i].TypeIndex;
                                    exit.Leg = Room[number].Exits[i].Leg;
                                    switch (exit.Type) {
                                ...
                                if (exit.Room == 0 || !EditGame.Logics.Contains(exit.Room)) {
                                    ...
                                    exit.Room = errptnum;
                                    ErrPt[errptnum].Visible = true;
                                    ErrPt[errptnum].Loc = GetInsertPos(Room[number].Loc);
                                    ...
                                    ErrPt[errptnum].Room = exit.Room;
                                    ErrPt[errptnum].FromRoom = number;
                                }
                            }
                        }
                        // any existing exits left need to be removed
                        for (int i = 0; i < Room[number].Exits.Count; i++) {
                            ExitType type = Room[number].Exits[i].Type;
                            int transfer = Room[number].Exits[i].TypeIndex;
                            // only erpt and transfer points are affected
                            switch (type) {
                            case ExitType.Transfer:
                                // remove this leg of transpt
                                ...
                                if (TransPt[transfer].Count == 0) {
                                    ...
                                    // determine which leg to clear
                                    int leg = Room[number].Exits[i].Leg;
                                    // clear this leg
                                    TransPt[transfer].ExitID[leg] = "";
                                    TransPt[transfer].Room[leg] = 0;
                                }
                        ...
                        Room[number].Exits.CopyFrom(tmpExits);
                                        ...
                                        for (int j = 0; j < Room[i].Exits.Count; j++) {
                                            ...
                                            if (Room[i].Exits[j].Room == number) {
                                                ...
                                                if (type == ExitType.Error) {
                                                    if (needpos) {
                                                        // move it here
                                                        Room[number].Loc.X = GridPos(ErrPt[transfer].Loc.X - 0.1f);
                                                        Room[number].Loc.Y = GridPos(ErrPt[transfer].Loc.Y - 0.1402f);
                                                        // only the first errpt is replaced by the new room
                                                        int order = ErrPt[transfer].Order;
                                                        objOrder[order].Number = number;
                                                        objOrder[order].Type = LayoutSelection.Room;
                                                        Room[number].Order = order;
                                                        needpos = false;
                                                    }
                                                    else {
                                                        // more than one matching errpt is just removed
                        ...
                        if (needpos) {
                            Room[number].Loc = GetInsertPos(
                                (picDraw.ClientSize.Width / 2 - Offset.X) / DSF,
                                (picDraw.ClientSize.Height / 2 - Offset.Y) / DSF);
                            objOrder[objCount].Type = LayoutSelection.Room;
                            objOrder[objCount].Number = number;
                            Room[number].Order = objCount;
                            objCount++;
                        }
                    }
                    else {
                        // hide the room
                        if (Room[number].Visible) {
                            // first, check for any existing transfers or errpts FROM the room being hidden
                            for (int i = 0; i < Room[number].Exits.Count; i++) {
                                ExitType type = Room[number].Exits[i].Type;
                                int transfer = Room[number].Exits[i].TypeIndex;
                            ...
                            // step through all other exits
                            bool adderrpt = !EditGame.Logics.Contains(number);
                            for (int i = 1; i < 256; i++) {
                                // only need to check rooms that are currently visible
                                if (i != number && Room[i].Visible) {
                                    for (int j = 0; j < Room[i].Exits.Count; j++) {
                                        if (Room[i].Exits[j].Room == number && Room[i].Exits[j].Status != ExitStatus.Deleted) {
                                            // check for transfer pt
                            ...
                            Room[number].Exits.Clear();
                            // remove from object list
                            CompactLoadList(Room[number].Order);
                        }
                        else {
            ...
            if (count != fileCount) {
                // add a warning
                warning = true;
                errorList.Add("The number of objects placed did not equal the number stored in the layout file.");
            }
            ...
            if (warning) {
                ...
                layoutData.Insert(0, "Layout Data File Error Report");
                layoutData.Insert(1, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
                layoutData.Insert(2, "");
                ...
                try {
                    ...
                    sw.Close();
                    // notify user that rebuild might be needed
                    linetext = "Errors were encountered in the version 2 layout file. WinAGI " +
                                "attempted to repair them.\nYou should make sure all rooms, " +
                                "exits and comments are correctly placed and  then save " +
                                "the layout. Consider using the 'Repair Layout' option if " +
                                "there are significant discrepancies.\n\nA list of specific " +
        ...
        internal void SaveLayout() {
            ...
            try {
                ...
                // add header
                writer.WriteLine(JsonSerializer.Serialize(layoutfile, jOptions));
                ...
                foreach (ObjInfo obj in ObjOrder) {
                    ...
                    case LayoutSelection.Room:
                        if (Room[i].Visible) {
                            ...
                            writer.WriteLine(JsonSerializer.Serialize(room as LayoutFileData, jOptions));
                        }
                        break;
                    case LayoutSelection.TransPt:
                        if (TransPt[i].Count > 0) {
                            ...
                            LFDTransPt transpt = new() {
                                ...
                                Visible = true,
                                Loc = TransPt[i].Loc,
                                Rooms = [TransPt[i].Room[0], TransPt[i].Room[1]],
                                ExitID = TransPt[i].ExitID,
                                Count = TransPt[i].Count
                            };
                            // add the room to the layout file
                            writer.WriteLine(JsonSerializer.Serialize(transpt as LayoutFileData, jOptions));
                        }
                        break;
                    case LayoutSelection.ErrPt:
                        if (ErrPt[i].Visible) {
                            ...
                            LFDErrPt errpt = new() {
                                ...
                                Visible = true,
                                Loc = ErrPt[i].Loc,
                                ExitID = ErrPt[i].ExitID,
                                FromRoom = ErrPt[i].FromRoom,
                                Room = ErrPt[i].Room
                            };
                            // add the room to the layout file
                            writer.WriteLine(JsonSerializer.Serialize(errpt as LayoutFileData, jOptions));
                        }
                        break;
                    case LayoutSelection.Comment:
                        if (Comment[i].Visible) {
                            ...
                            // add the room to the layout file
                            writer.WriteLine(JsonSerializer.Serialize(comment as LayoutFileData, jOptions));
                        }
                        break;
                    }
                }
        ...
        private static Exits ParseExits(string[] exitdata) {
            ...
            for (int i = 7; i < exitdata.Length; i++) {
                ...
                if (exit.Length == 6) {
                    ...
                    int id = exit[0].IntVal();
                    int room = exit[1].IntVal();
                    ExitReason reason = (ExitReason)exit[2].IntVal();
                    int style = exit[3].IntVal();
                    int transfer = exit[4].IntVal();
        ...
        private bool ExtractLayout(bool SavePos = false) {
            ...
            var progress = new frmProgress(MDIMain) {
                Text = SavePos ? "Repairing Layout" : "Extract Layout"
            };
            ...
            for (i = 1; i <= 255; i++) {
                ...
                ELRoom[stack[stackCount]].Group = group;
                groupCount[group] = 1;

                while (stackCount != 0) {
                    room = stack[stackCount];
                    stackCount--;

                    if (Room[room].Visible) {
                        // check exits
                        for (j = 0; j <= 3; j++) {
                            // if target room is valid and target does not yet have a group
                            if (ELRoom[room].Exits[j] > 0 && ELRoom[ELRoom[room].Exits[j]].Group == 0) {
                                ELRoom[ELRoom[room].Exits[j]].Group = group;
                                groupCount[group]++;
                                stackCount++;
                                stack[stackCount] = ELRoom[room].Exits[j];
                            }
                        }
                        // check entrances
                        for (j = 0; j <= 3; j++) {
                            // if from room is valid and from room does not yet have a group
                            if (ELRoom[room].Enter[j] > 0 && ELRoom[ELRoom[room].Enter[j]].Group == 0) {
                                ELRoom[ELRoom[room].Enter[j]].Group = group;
                                groupCount[group]++;
                                stackCount++;
                                stack[stackCount] = ELRoom[room].Enter[j];
                            }
                        }
                    }
                }
                ...
                for (i = 1; i <= group; i++) {
                    if (groupCount[i] > 1) {
                        ...
                        while (qEnd != qPos) {
                            ...
                            if (!ELRoom[queue[qPos]].Analyzed) {
                                // mark it as analyzed
                                ELRoom[queue[qPos]].Analyzed = true;
                                ...
                                for (int dir = 0; dir <= 3; dir++) {
                                    ...
                                    while (stackCount != 0) {
                                        ...
                                        stackCount--;
                                        // Exits
                                        if (ELRoom[room].Exits[dir] != 0 && !ELRoom[ELRoom[room].Exits[dir]].Placed) {
                                            // if space already occupied
                                            if (ItemAtPoint(Room[room].Loc.X + DeltaX(dir), Room[room].Loc.Y + DeltaY(dir), i)) {
                                                // try next space clockwise
                                                if (!ItemAtPoint(Room[room].Loc.X + DeltaX(dir) + DeltaX(dir + 1), Room[room].Loc.Y + DeltaY(dir) + DeltaY(dir + 1), i)) {
                                                    Room[ELRoom[room].Exits[dir]].Loc.X = Room[room].Loc.X + DeltaX(dir) + DeltaX(dir + 1);
                                                    Room[ELRoom[room].Exits[dir]].Loc.Y = Room[room].Loc.Y + DeltaY(dir) + DeltaY(dir + 1);
                                                    // try next space counterclockwise
                                                }
                                                else if (!ItemAtPoint(Room[room].Loc.X + DeltaX(dir) + DeltaX(dir + 3), Room[room].Loc.Y + DeltaY(dir) + DeltaY(dir + 3), i)) {
                                                    Room[ELRoom[room].Exits[dir]].Loc.X = Room[room].Loc.X + DeltaX(dir) + DeltaX(dir + 3);
                                                    Room[ELRoom[room].Exits[dir]].Loc.Y = Room[room].Loc.Y + DeltaY(dir) + DeltaY(dir + 3);
                                                }
                                                else {
                                                    // locate free point around target
                                                    tmpCoord = GetInsertPos(Room[room].Loc.X + DeltaX(dir), Room[room].Loc.Y + DeltaY(dir), i);
                                                    Room[ELRoom[room].Exits[dir]].Loc.X = tmpCoord.X;
                                                    Room[ELRoom[room].Exits[dir]].Loc.Y = tmpCoord.Y;
                                                }
                                            }
                                            else {
                                                Room[ELRoom[room].Exits[dir]].Loc.X = Room[room].Loc.X + DeltaX(dir);
                                                Room[ELRoom[room].Exits[dir]].Loc.Y = Room[room].Loc.Y + DeltaY(dir);
                                            }

                                            // set the group, mark it and add to stack
                                            ELRoom[ELRoom[room].Exits[dir]].Group = (byte)i;
                                            ELRoom[ELRoom[room].Exits[dir]].Placed = true;
                                            stackCount++;
                                            stack[stackCount] = ELRoom[room].Exits[dir];
                                            // if not already analyzed
                                            if (!ELRoom[ELRoom[room].Exits[dir]].Analyzed) {
                                                // add to queue as well
                                                queue[qEnd] = ELRoom[room].Exits[dir];
                                                qEnd++;
                                            }
                                            // update min and Max
                                            UpdateMinMax(Room[ELRoom[room].Exits[dir]].Loc);
                                        }
                                        // Entrances
                                        if (ELRoom[room].Enter[dir] != 0 && !ELRoom[ELRoom[room].Enter[dir]].Placed) {
                                            // if space already occupied
                                            if (ItemAtPoint(Room[room].Loc.X + DeltaX(dir), Room[room].Loc.Y + DeltaY(dir), i)) {
                                                // try next space clockwise
                                                if (!ItemAtPoint(Room[room].Loc.X + DeltaX(dir) + DeltaX(dir + 1), Room[room].Loc.Y + DeltaY(dir) + DeltaY(dir + 1), i)) {
                                                    Room[ELRoom[room].Enter[dir]].Loc.X = Room[room].Loc.X + DeltaX(dir) + DeltaX(dir + 1);
                                                    Room[ELRoom[room].Enter[dir]].Loc.Y = Room[room].Loc.Y + DeltaY(dir) + DeltaY(dir + 1);
                                                    // try next space counterclockwise
                                                }
                                                else if (!ItemAtPoint(Room[room].Loc.X + DeltaX(dir) + DeltaX(dir + 3), Room[room].Loc.Y + DeltaY(dir) + DeltaY(dir + 3), i)) {
                                                    Room[ELRoom[room].Enter[dir]].Loc.X = Room[room].Loc.X + DeltaX(dir) + DeltaX(dir + 3);
                                                    Room[ELRoom[room].Enter[dir]].Loc.Y = Room[room].Loc.Y + DeltaY(dir) + DeltaY(dir + 3);
                                                }
                                                else {
                                                    // locate free point around target
                                                    tmpCoord = GetInsertPos(Room[room].Loc.X + DeltaX(dir), Room[room].Loc.Y + DeltaY(dir), i);
                                                    Room[ELRoom[room].Enter[dir]].Loc.X = tmpCoord.X;
                                                    Room[ELRoom[room].Enter[dir]].Loc.Y = tmpCoord.Y;
                                                }
                                            }
                                            else {
                                                Room[ELRoom[room].Enter[dir]].Loc.X = Room[room].Loc.X + DeltaX(dir);
                                                Room[ELRoom[room].Enter[dir]].Loc.Y = Room[room].Loc.Y + DeltaY(dir);
                                            }
                                            // set the group, mark it and add to stack
                                            ELRoom[ELRoom[room].Enter[dir]].Group = (byte)i;
                                            ELRoom[ELRoom[room].Enter[dir]].Placed = true;
                                            stackCount++;
                                            stack[stackCount] = ELRoom[room].Enter[dir];
                                            // if not already analyzed
                                            if (!ELRoom[ELRoom[room].Enter[dir]].Analyzed) {
                                                // add to queue as well
                                                queue[qEnd] = ELRoom[room].Enter[dir];
                                                qEnd++;
                                            }
                                            // update min and Max
                                            UpdateMinMax(Room[ELRoom[room].Enter[dir]].Loc);
                                        }
                                        // Other exits (only once though???)
                                        for (int idx = 0; idx < Room[room].Exits.Count; idx++) {
                                            if (Room[room].Exits[idx].Reason == ExitReason.Other) {
                                                // if other exit is a single room and room is not zero
                                                if (groupCount[ELRoom[Room[room].Exits[idx].Room].Group] == 1 && Room[room].Exits[idx].Room != 0) {
                                                    // insert the room
                                                    tmpCoord = GetInsertPos(Room[EditGame.Logics[room].Number].Loc, i);
                                                    Room[Room[room].Exits[idx].Room].Loc = tmpCoord;
                                                    UpdateMinMax(Room[Room[room].Exits[idx].Room].Loc);
                                                    ELRoom[Room[room].Exits[idx].Room].Placed = true;
                                                    // move to this group
                                                    groupCount[ELRoom[Room[room].Exits[idx].Room].Group] = 0;
                                                    ELRoom[Room[room].Exits[idx].Room].Group = (byte)i;
                                                    groupCount[i]++;
                                                }
                                            }
                                        }
            ...
            for (int i = 0; i < Room[ToRoom].Exits.Count; i++) {
                if (Room[ToRoom].Exits[i].Reason == Dir && Room[ToRoom].Exits[i].Room == FromRoom) {
                    ...
                    if (Room[ToRoom].Exits[i].Type == ExitType.Transfer) {
                        int transptnum = Room[ToRoom].Exits[i].TypeIndex;
                        if (TransPt[transptnum].Count == 1) {
                            return transptnum;
                        }
                    }
                }
        ...
        private void InsertTransfer(ref TSel NewSel) {
            ...
            Exit exit = Room[NewSel.Number].Exits[NewSel.ExitID];
            // dx and dy
        ...
        internal void UpdateLayout(UpdateReason Reason, int LogicNumber, Exits NewExits, int NewNum = 0) {
            // 
            ...
            case UpdateReason.RenumberRoom:
                ...
                if (idx != -1) {
                    ...
                    MessageBox.Show(MDIMain,
                        "Invalid index in UpdateLayout(). Layout may be corrupt.\n\n" +
                        $"({Reason}, {LogicNumber})",
                        "Layout Error",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Error);
        ...
        internal void UpdatePictureStatus(int LogicNumber, bool ShowPic) {
        ...
        /// <summary>
        /// Repairs the layout by recalculating all exits based on current game
        /// logic information. Room and comment positions don't get affected;
        ...
        /// <summary>
        /// Updates the text for a logic to match current exit status of the layout.
        /// </summary>
        /// <param name="logic"></param>
        private void UpdateLogicCode(Logic logic) {
            // called by save layout; at least one exit needs updating or
            // this method will not have been called

            // ensure source is loaded
            bool logLoad = logic.Loaded;
            if (!logLoad) {
                logic.Load();
            }
            ...
            for (int loop = 0; loop < 2; loop++) {
                ...
                if (loop == 0) {
                    ...
                    sourcetext = Regex.Replace(logic.SourceText, @"\r\n?|\n", Environment.NewLine);
                    ...
                    sourcetext = LogicEditors[index].fctb.Text;
                ...
                for (int i = Room[logic.Number].Exits.Count - 1; i >= 0; i--) {
                    // reset ok flag
                    exitOK = false;
                    bool addAsNew = false;
                    ...
                    if (Room[logic.Number].Exits[i].Status == ExitStatus.OK) {
                        // ok; need to verify that the exitID has not changed
                        idpos = sourcetext.IndexOf("##" + Room[logic.Number].Exits[i].ID + "##");
                        // if found
                        if (idpos != -1) {
                            // get the line start and end (adjust for width of cr-lf)
                            linestart = sourcetext.LastIndexOf(newlinechar, idpos);
                            if (linestart == -1) {
                                linestart = 0;
                            }
                            else {
                            ...
                            // new.room(xxx); [ ##exitID##
                            // find new.room cmd,
                            newrmpos = FindTokenPosRev(sourcetext, "new.room", idpos);
                            // if found,
                            if (newrmpos != -1) {
                                // verify on same line
                                if (linestart < newrmpos) {
                                    ...
                                    commentpos = newrmpos;
                                    Exit tmpExit = AnalyzeExit(sourcetext, ref commentpos);

                                    // if reason and room match
                                    if (tmpExit.Reason == Room[logic.Number].Exits[i].Reason &&
                                        tmpExit.Room == Room[logic.Number].Exits[i].Room) {
                                        // exit is ok
                                        exitOK = true;
                                        // make sure exit style match the logic
                                        Room[logic.Number].Exits[i].Style = tmpExit.Style;
                                    }
                                }
                            }
                        }
                    ...
                    if (Room[logic.Number].Exits[i].Status == ExitStatus.Changed) {
                            ...
                            if (newrmpos != -1) {
                                ...
                                if (linestart < newrmpos) {
                                    ...
                                    if (tmpExit.Reason == Room[logic.Number].Exits[i].Reason) {
                                        ...
                                        idpos = FindTokenPos(sourcetext, ")", newrmpos);

                                        // if found,
                                        if (idpos != -1) {
                                            // insert new room here
                                            if (EditGame.IncludeIDs) {
                                                sourcetext = sourcetext.Left(newrmpos + 1) + EditGame.Logics[Room[logic.Number].Exits[i].Room].ID + sourcetext.Right(sourcetext.Length - idpos);
                                            }
                                            else {
                                                sourcetext = sourcetext.Left(newrmpos + 1) + Room[logic.Number].Exits[i].Room + sourcetext.Right(sourcetext.Length - idpos);
                                            }
                        ...
                        if (exitOK) {
                            ...
                            if (loop == 1 || index == -1) {
                                ...
                                Room[logic.Number].Exits[i].Status = ExitStatus.OK;
                            }
                        }
                        else {
                            // something was wrong with this exit; ignore
                    ...
                    if (addAsNew || Room[logic.Number].Exits[i].Status == ExitStatus.NewExit) {
                        if (insertpos == -1) {
                            // find last return cmd
                            insertpos = FindTokenPosRev(sourcetext, "return");
                            // if not found
                            if (insertpos == -1) {
                                // add to end
                                sourcetext += newlinechar;
                                insertpos = sourcetext.Length;
                            }
                            else {
                                // now move to beginning of line
                                insertpos = sourcetext.LastIndexOf(newlinechar, insertpos);
                                if (insertpos == -1) {
                                    // add to beginning
                                    insertpos = 0;
                                }
                                else {
                                    insertpos += newlinechar.Length;
                                }
                            }
                        }
                        if (insertpos == 0) {
                            // add to beginning
                            sourcetext = NewExitText(Room[logic.Number].Exits[i], newlinechar) + sourcetext;
                        }
                        else {
                            // add at insertpos
                            sourcetext = sourcetext.Left(insertpos) + NewExitText(Room[logic.Number].Exits[i]) + sourcetext.Right(sourcetext.Length - insertpos);
                        }
                    ...
                    if (Room[logic.Number].Exits[i].Status == ExitStatus.Deleted) {
                        ...
                        if (idpos != -1) {
                            ...
                            string newText = "[ DELETED BY LAYOUT EDITOR " + newlinechar + "[ ";
                            commentpos = sourcetext.IndexOf('[', linestart);
                        ...
                        if (loop == 1 || index == -1) {
                            ...
                            Room[logic.Number].Exits.Remove(i);
                        }
                    }
                }
                ...
                if (loop == 0) {
                    ...
                    LogicEditors[index].fctb.Text = sourcetext;
        ...
        /// <summary>
        /// Draws the layout, but only if the specified object is on screen.
        /// </summary>
        /// <param name="objType"></param>
        /// <param name="objNumber"></param>
        internal void DrawLayout(LayoutSelection objType, int objNumber) {
                ...
                case LayoutSelection.Room:
                    ...
                    if (Room[roomnum].Visible) {
                        ...
                        if (ObjOnScreen(ObjOrder[i])) {
                            ...
                            x1 = LayoutToScreenX(Room[roomnum].Loc.X);
                            y1 = LayoutToScreenY(Room[roomnum].Loc.Y);
                            w = RM_SIZE * DSF;
                            h = RM_SIZE * DSF;
                            g.FillRectangle(objBrush, x1, y1, w, h);
                            g.DrawRectangle(objPen, x1, y1, w, h);
                            // if showing pic for this room, draw the matching vis pic
                            if (Room[roomnum].ShowPic) {
                                DrawRoomPic(g, roomnum);
                            }
                            logicID = ResourceName(EditGame.Logics[roomnum], true, true);
                            SizeF textSize = g.MeasureString(logicID, layoutFont);
                            if (textSize.Width <= RM_SIZE * DSF) {
                                tX = (Room[roomnum].Loc.X + RM_SIZE / 2) * DSF + Offset.X - textSize.Width / 2;
                                tY = (Room[roomnum].Loc.Y + 0.7f) * DSF + Offset.Y - 3 * textSize.Height / 2;
                                g.DrawString(logicID, layoutFont, textBrush, tX, tY);
                            }
                            else {
                                // if logic id is too long, it won't fit in the box;
                                // split across two lines, and truncate if necessary
                                List<string> lines = WordWrapLines(logicID, layoutFont, RM_SIZE * DSF, g, []);
                    ...
                    if (ErrPt[ObjOrder[i].Number].Visible) {
                        if (ObjOnScreen(ObjOrder[i])) {
                            ...
                            [
                                ...
                                new PointF((ErrPt[ObjOrder[i].Number].Loc.X + ERRPT_SIZE) * DSF + Offset.X,
                                ErrPt[ObjOrder[i].Number].Loc.Y * DSF + Offset.Y),
                                new PointF((ErrPt[ObjOrder[i].Number].Loc.X + ERRPT_SIZE / 2) * DSF + Offset.X,
                                (ErrPt[ObjOrder[i].Number].Loc.Y + 0.866f * ERRPT_SIZE) * DSF + Offset.Y)
                            ];
                            using GraphicsPath path = new GraphicsPath();
                            float radius = 5.0f * DSF / 40; // radius for rounded corners
                                                            // Create rounded triangle path
                            ...
                            tX = (ErrPt[ObjOrder[i].Number].Loc.X + ERRPT_SIZE / 2) * DSF + Offset.X - textSize.Width / 2;
                            tY = (ErrPt[ObjOrder[i].Number].Loc.Y + ERRPT_SIZE / 4) * DSF + Offset.Y - textSize.Height / 2;
        ...
        private void DrawCmtBox(Graphics g, int commentid) {
            // draws a rounded corner rectangle; converts layout coordinates into drawing surface pixel coordinates

            using Pen borderPen = new(WinAGISettings.CmtEdgeColor.Value, 2);
            float radius = 5.0f * DSF / 40; // radius for rounded corners
            RectangleF rect = new(
            Comment[commentid].Loc.X * DSF + Offset.X,
            Comment[commentid].Loc.Y * DSF + Offset.Y,
            ...
            // best way to make sure the image on the draw surface matches that of the 
            // comment textbox is to copy the image as a bitmap then blit it onto the 
            // surface
            ...
            // text box never matches the spacing when drawing on the graphics
            // object (using DrawString), even if I match the font exactly.
            ...
            //    //Point pt = new();
            //    //pt.X = (int)Math.Round(tX);
            //    //pt.Y = (int)Math.Round(tY);
            //    //TextRenderer.DrawText(g, line, layoutFont, pt, WinAGISettings.CmtEdgeColor.Value);
            //    tY += textHeight;
            //}
        }
            ...
            case LayoutSelection.TransPt:
                if (SecondLeg) {
                    objRect = new(new(TransPt[objnum].Loc[1].X * DSF + Offset.X,
                        ...
                        new(TRANSPT_SIZE * DSF, TRANSPT_SIZE * DSF));
                }
                else {
        ...
        /// <summary>
        /// Returns true if any part of the line is on the draw surface client
        /// area.
        /// </summary>
        /// <param name="objtype"></param>
        /// <param name="objnum"></param>
        ...
        private void DrawRoomPic(Graphics g, int RoomNum) {
            ...
            bool unloaded = !EditGame.Pictures[RoomNum].Loaded;
            if (unloaded) {
        ...
        /// <summary>
        /// Populates the SelectedObjects collection with all objects that
        /// are within the drag area. If more than one object is selected,
        /// sets selection to Multiple; if only one, sets selection to that
        /// object; if none, sets selection to None.
        /// </summary>
        ...
        private void GetSelectedObjects(RectangleF dragarea) {
            ...
            for (int i = 0; i < ObjOrder.Count; i++) {
                ...
                case LayoutSelection.ErrPt:
                    ...
                    if (IsInSelection(ErrPt[ObjOrder[i].Number].Loc, ERRPT_SIZE, 0.866f * ERRPT_SIZE)) {
                        // add it
                        ObjInfo tmp = new(ObjOrder[i].Type, ObjOrder[i].Number, ExitLeg.SecondLeg);
                        SelectedObjects.Add(tmp);
                    }
        ...
        private void SelectExit(TSel newexit) {
            ...
            Selection = new TSel {
                ...
                Type = LayoutSelection.Exit,
                Number = newexit.Number,
                ExitID = newexit.ExitID,
                Leg = newexit.Leg,
            ...
            // write to status bar
            spRoom1.Text = (Selection.Direction == ExitDirection.BothWays ? "" : "From: ")
            ...
            if (Room[Selection.Number].Exits[Selection.ExitID].Type == ExitType.Error) {
                ...
                if (Room[Selection.Number].Exits[Selection.ExitID].Room == 0) {
                    ...
                    spRoom2.Text = (Selection.Direction == ExitDirection.BothWays ? "" : "To: ")
            ...
            case ExitDirection.BothWays:
                ...
                string exitID = "";
                if (IsTwoWay(Selection.Number, Selection.ExitID, ref Room[Selection.Number].Exits[Selection.ExitID].Room, ref exitID)) {
                    // if it really is a twoway- this gets the second exit ID
                    exitID = "/" + exitID;
                }
                // now add the first
                exitID = Selection.ExitID + exitID;
        ...
        private void SetScrollBars() {
            ...
            // max must be big enought to allow setting LargeChange to 0.8 of clientwidth/height
            // based on empirical data, max LC value is SBMax - 107 pixels; to make sure we
            // can set it to 0.8 of SBMax, SBMax must be >= 535
            hScrollBar1.Maximum = Math.Max(535, picDraw.ClientSize.Width);
            vScrollBar1.Maximum = Math.Max(535, picDraw.ClientSize.Height);
        ...
        private void InsertErrPt(int FromRoom, int ExitIndex, int ErrRoom) {
            ...
            string fromID;
            if (EditGame.Logics.Contains(FromRoom)) {
                fromID = EditGame.Logics[FromRoom].ID;
            }
            else {
                fromID = "logic " + FromRoom;
            }

            // inform user that an error point is being inserted
            string msgText = "Exit " + Room[FromRoom].Exits[ExitIndex].ID + " in '" +
                fromID + "' points to a nonexistent room (" +
            ...
            MDIMain.MsgBoxWithHelp(
                ...
                MessageBoxButtons.OK,
                MessageBoxIcon.Information,
                "htm\\winagi\\editor_layout.htm#errpts");
            ...
            Room[FromRoom].Exits[ExitIndex].TypeIndex = errptNum;
            ...
            ErrPt[errptNum].Loc.X = tmpCoord.X + (RM_SIZE - ERRPT_SIZE) / 2;
            ErrPt[errptNum].Loc.Y = tmpCoord.Y + (RM_SIZE - 0.866f * ERRPT_SIZE) / 2;
            ErrPt[errptNum].Visible = true;
            ErrPt[errptNum].Room = ErrRoom;
            ErrPt[errptNum].FromRoom = FromRoom;
            ErrPt[errptNum].ExitID = Room[FromRoom].Exits[ExitIndex].ID;
            ErrPt[errptNum].Order = ObjOrder.Count;
            ObjOrder.Add(new(LayoutSelection.ErrPt, errptNum, 0));
        }
                                ...
                                if (Room[i].Exits[j].Type != ExitType.Transfer) {
                                    ...
                                    if (LineOnScreen(sp, ep)) {
                                        ...
                                        if (PointOnArrow(pos, sp, ep)) {
                                            ...
                                            if (IsTwoWay(i, j)) {
                                                retval.Direction = ExitDirection.OneWay;
                                            }
                                            else {
                                                retval.Direction = ExitDirection.Single;
                                            }
                                            // select this line
                                        ...
                                        // if not on arrow, check line
                                            ...
                                            if (IsTwoWay(i, j, ref tmpRoom, ref tmpID)) {
                                                ...
                                                if (PointOnArrow(pos, ep, sp)) {
                                                    ...
                                                    // both
                                                    retval.Direction = ExitDirection.BothWays;
                                                }
                                            }
                                            else {
                                                // single line
        ...
        private bool PointOnHandle(Point pos, TSel testobj, out int handleID) {
            ...
            case LayoutSelection.ErrPt:
                ...
                rect.Width = ERRPT_SIZE * DSF;
                rect.Height = 0.866f * ERRPT_SIZE * DSF;
                break;
            case LayoutSelection.Comment:
            ...
            case LayoutSelection.ErrPt:
                ...
                using (GraphicsPath path = new GraphicsPath()) {
                    ...
                    [
                        ...
                        new PointF((ErrPt[testobj.Number].Loc.X + ERRPT_SIZE) * DSF + Offset.X,
                            ErrPt[testobj.Number].Loc.Y * DSF + Offset.Y),
                        new PointF((ErrPt[testobj.Number].Loc.X + ERRPT_SIZE / 2) * DSF + Offset.X,
                            (ErrPt[testobj.Number].Loc.Y + 0.866f * ERRPT_SIZE) * DSF + Offset.Y)
                    ];
                    // Create rounded triangle path
                    path.AddArc(errPoints[0].X + radius * 0.366f, errPoints[0].Y, radius, radius, 150, 120);
                    path.AddArc(errPoints[1].X - radius * 1.366f, errPoints[1].Y, radius, radius, 270, 120);
        ...
        private bool SameAsSelection(TSel tmpSel) {
            ...
            return Selection.Type == tmpSel.Type &&
                   ...
                   Selection.Direction == tmpSel.Direction;
        }
        ...
        private void RecalculateMaxMin() {
            ...
            for (int i = 0; i < ObjOrder.Count; i++) {
                int objNum = ObjOrder[i].Number;
                switch (ObjOrder[i].Type) {
                case LayoutSelection.Room:
                    CheckMaxMin(Room[objNum].Loc.X, Room[objNum].Loc.Y, RM_SIZE, RM_SIZE);
                    break;
                case LayoutSelection.TransPt:
                    CheckMaxMin(TransPt[objNum].Loc[0].X, TransPt[objNum].Loc[0].Y, RM_SIZE / 2, RM_SIZE / 2);
                    CheckMaxMin(TransPt[objNum].Loc[1].X, TransPt[objNum].Loc[1].Y, RM_SIZE / 2, RM_SIZE / 2);
                    break;
                case LayoutSelection.ErrPt:
                    CheckMaxMin(ErrPt[objNum].Loc.X, ErrPt[objNum].Loc.Y, ERRPT_SIZE, 0.866f * ERRPT_SIZE);
                    break;
                case LayoutSelection.Comment:
                    CheckMaxMin(Comment[objNum].Loc.X, Comment[objNum].Loc.Y, Comment[objNum].Size.Width, Comment[objNum].Size.Height);
                    break;
                }
            }
            // if no objects,
        ...
        /// <summary>
        /// This method is called by the main form when a logic resource is
        /// chosen on the treelist; this sub then unselects the current 
        /// selection and then selects RoomNum, repositioning the drawing
        ...
        private void ShowRoom() {
            ...
            if (frm.ShowDialog(MDIMain) == DialogResult.OK) {
                ...
                Exits tmpExits = ExtractExits(EditGame.Logics[frm.NewResNum]);
                // update layout file
        ...
        private TSel ChangeFromRoom(TSel OldSel, int FromRoom) {
            ...
            if (IsTwoWay(OldSel.Number, OldSel.ExitID)) {
                ...
                OldSel.Direction = ExitDirection.OneWay;
            }
            else {
                OldSel.Direction = ExitDirection.Single;
            }
        ...
        private TSel ChangeToRoom(TSel OldSel, int NewRoom, LayoutSelection ObjType) {
            ...
            Exit exit = Room[OldSel.Number].Exits[OldSel.ExitID];
        ...
        private static string NewExitText(Exit NewExit, string endofline = "\r\n") {
        ...
        private void DeleteExit(TSel OldSel) {
            ...
            case ExitType.Transfer:
                ...
                if (TransPt[index].Count == 1 || OldSel.Direction == ExitDirection.BothWays) {
            ...
            if (OldSel.Direction == ExitDirection.BothWays) {
        ...
        private void SetExitPos(int Index, string exitID) {
            ...
            Exit exit = Room[Index].Exits[exitID];
            ...
            if (exit.Type == ExitType.Error) {
                ...
                exit.EP.X += ERRPT_SIZE / 2;
                exit.EP.Y += ERRPT_SIZE * 0.2887f;
                switch (exit.Reason) {
                case ExitReason.Horizon:
                ...
                [
                    new PointF(ErrPt[index].Loc.X,
                                ErrPt[index].Loc.Y),
                                new PointF(ErrPt[index].Loc.X + ERRPT_SIZE,
                                ErrPt[index].Loc.Y),
        ...
        private static PointF? FindIntersection(GraphicsPath path, PointF p1, PointF p2) {
        ...
        private static bool IsBehind(PointF TestLoc, SizeF TestSize, PointF TopLoc, SizeF TopSize) {
        ...
        private static float GridPos(float value) {
        ...
        internal void ShowHelp() {
            ...
            case LayoutTool.Select:
                switch (Selection.Type) {
                case LayoutSelection.None:
                    topic = "htm\\winagi\\editor_layout.htm";
                    break;
                case LayoutSelection.Room:
                    topic = "htm\\winagi\\editor_layout.htm#rooms";
                    break;
                case LayoutSelection.Exit:
                    topic = "htm\\winagi\\editor_layout.htm#exits";
                    break;
                case LayoutSelection.TransPt:
                    topic = "htm\\winagi\\editor_layout.htm#transpts";
                    break;
                case LayoutSelection.Comment:
                    topic = "htm\\winagi\\editor_layout.htm#comments";
                    break;
                case LayoutSelection.ErrPt:
                    topic = "htm\\winagi\\editor_layout.htmerrpts";
                    break;
                case LayoutSelection.Multiple:
                    topic = "htm\\winagi\\editinglayouts.htm#moverooms";
                    break;
                }
                break;
            case LayoutTool.Edge1:
            case LayoutTool.Edge2:
            case LayoutTool.Other:
                topic = "htm\\winagi\\editinglayouts.htm#addexits";
                break;
            case LayoutTool.Room:
                topic = "htm\\winagi\\editinglayouts.htm#addrooms";
                break;
            case LayoutTool.Comment:
                topic = "htm\\winagi\\editinglayouts.htm#comments";
                break;
            }
