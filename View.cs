...
namespace WinAGI.Engine {
    ...
    public class View : AGIResource {
        ...
        private void CompileView() {
            ...
            loopOffset = new int[mLoopCol.Count];
            ...
            for (i = 0; i < mLoopCol.Count; i++) {
                ...
                if (mLoopCol[i].Mirrored) {
                    mirrorAdded = (mLoopCol[i].MirrorLoop < i);
                }
                else {
                    mirrorAdded = false;
                }
                if (mirrorAdded) {
                    ...
                    loopOffset[i] = loopOffset[mLoopCol[i].MirrorLoop];
                }
                else {
                    loopOffset[i] = Pos;
                    WriteByte((byte)mLoopCol[i].Cels.Count);
                    celOffset = new int[mLoopCol[i].Cels.Count];
                    ...
                    for (j = 0; j < mLoopCol[i].Cels.Count; j++) {
                        ...
                        if (mLoopCol[i].Mirrored) {
                            ...
                            transCol = (byte)(0x80 + i * 0x10 + mLoopCol[i][j].TransColor);
                        }
                        else {
                            // just use transparent color
                            transCol = (byte)mLoopCol[i][j].TransColor;
                        }
                        WriteByte(transCol);
                        // cel data
                        celData = CompressedCel(mLoopCol[i][j], mLoopCol[i].Mirrored);
                        for (k = 0; k < celData.Length; k++) {
                            WriteByte(celData[k]);
                        }
                    }
                    // add cel offsets
                    for (j = 0; j < mLoopCol[i].Cels.Count; j++) {
                        WriteWord((ushort)celOffset[j], loopOffset[i] + 1 + 2 * j);
                    }
            ...
            for (i = 0; i < mLoopCol.Count; i++) {
                WriteWord((ushort)loopOffset[i], 5 + 2 * i);
            }
        ...
        bool ExpandCelData(int startPos, Cel cel) {
            ArgumentNullException.ThrowIfNull(cel);
            bool retval = true;
            byte celY = 0;
            byte byteval;
            byte chunkColor, chunkCount;
            byte width = cel.Width, height = cel.Height;
            byte transpColor = (byte)cel.TransColor;
            ...
            do {
                ...
                do {
                    ...
                    if (byteval > 0) {
                        // extract color
                        chunkColor = (byte)(byteval / 0x10);
                        chunkCount = (byte)(byteval % 0x10);
                        // add this color for correct number of pixels
                        for (int i = 0; i < chunkCount; i++) {
                            celData[celX++, celY] = chunkColor;
                        }
                    }
                ...
                while (celX < width) {
                    celData[celX++, celY] = transpColor;
                }
                celY++;
            } while (celY < height);
            cel.AllCelData = celData;
            return retval;
        }
                    ...
                    if (descriptionoffset < mSize - 1) {
                        ...
                        do {
                            ...
                            if ((nextchar > 0) && (mViewDesc.Length < 255)) {
                                mViewDesc += Encoding.GetEncoding(CodePage).GetString([nextchar]);
                            }
                        }
