...
namespace WinAGI.Editor {
    public partial class frmPlotEdit : Form {
        ...
        private void frmPlotEdit_HelpRequested(object sender, HelpEventArgs hlpevent) {
            string topic = "htm\\winagi\\manipulatecommands.htm#coord_dlg";
            Help.ShowHelp(Base.HelpParent, Base.WinAGIHelp, HelpNavigator.Topic, topic);
        ...
        private void DrawPlotPoint(Graphics g) {
            ...
            if (SelCmd.Pen.PlotShape == PlotShape.Circle) {
                ...
                for (int Y = 0; Y <= SelCmd.Pen.PlotSize * 2; Y++) {
                    for (int X = 0; X <= SelCmd.Pen.PlotSize; X++) {
                        newpatt = (byte)(oldpatt >> 1);
                        if (oldpatt % 2 == 1) {
                            newpatt ^= 0xB8;
                        }
