...
namespace WinAGI.Editor {
    public partial class frmObjectEdit : Form {
        ...
        private void frmObjectEdit_HelpRequested(object sender, HelpEventArgs hlpevent) {
            ShowHelp();
        ...
        private void mnuEFindInLogic_Click(object sender, EventArgs e) {
            ...
            string itemtext = (string)fgObjects[1, fgObjects.CurrentCell.RowIndex].Value;
            if (itemtext == "?") {
                return;
            }
            frmFind.ResetSearch();
            ...
            if (txtMaxScreenObjs.Text.Length > 0) {
                ...
                if (newMax != EditInvList.MaxScreenObjects) {
                    if (newMax == 0) {
                        ...
                        MessageBoxButtons.YesNo,
                        MessageBoxIcon.Question,
                        "htm\\agi\\screenobjs.htm#maxscreenobjs") == DialogResult.No) {
                            e.Cancel = true;
                            return;
                        }
                    }
                            ...
                            for (int i = 0; i < EditInvList.Count; i++) {
                                if (i != fgObjects.CurrentCell.RowIndex) {
                                    if (EditInvList[i].ItemName == EditTextBox.Text) {
                                        ...
                                        WinAGISettings.WarnDupObj.Value = !noWarn;
                                        if (!WinAGISettings.WarnDupObj.Value) {
        ...
        public void ReplaceAll(string FindText, string ReplaceText, bool MatchWord, bool MatchCase) {
            ...
            if (count == 0) {
                ...
                AddUndo(NextUndo);
                MessageBox.Show(MDIMain,
                "The specified region has been searched. " + count + " replacements were made.",
                "Replace All",
                MessageBoxButtons.OK,
                MessageBoxIcon.Information);
            }
        ...
        private void SetEncryption(bool encrypt, bool DontUndo = false) {
            ...
            if (InGame && !DontUndo) {
                bool correct = !(EditGame.InterpreterVersion.Index == AGIVersion.v2089 || EditGame.InterpreterVersion.Index == AGIVersion.v2272);
                bool noWarn = false;
                if (correct != encrypt && WinAGISettings.WarnEncrypt.Value) {
                    DialogResult rtn = MsgBoxEx.Show(MDIMain,
                        "The target Interpreter Version for this game needs the OBJECT file to be " +
                        (correct ? "ENCRYPTED" : "UNENCRYPTED") + ". Are you sure you want to change it?",
            ...
            if (InGame) {
                ...
                if (WinAGISettings.AutoUpdateObjects.Value == AskOption.Ask) {
                    rtn = MsgBoxEx.Show(MDIMain,
                       ...
                       MessageBoxButtons.YesNo,
                       MessageBoxIcon.Question,
                       "Always take this action when saving the object list.", ref dontAsk);
                    if (dontAsk) {
                        if (rtn == DialogResult.Yes) {
                            WinAGISettings.AutoUpdateObjects.Value = AskOption.Yes;
                        }
                        else {
                            WinAGISettings.AutoUpdateObjects.Value = AskOption.No;
                        }
                        WinAGISettings.AutoUpdateObjects.WriteSetting(WinAGISettingsFile);
                    }
                }
                else {
                    if (WinAGISettings.AutoUpdateObjects.Value == AskOption.Yes) {
        ...
        public void EditProperties() {
            string description = EditInvList.Description;
            string id = "";
            if (GetNewResID(AGIResType.Objects, -1, ref id, ref description, InGame, 2)) {
                EditInvList.Description = description;
