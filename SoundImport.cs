...
namespace WinAGI.Engine {
    ...
    public static class SoundImport {
            ...
            for (i = 0; i < lines.Length; i++) {
                ...
                if (linetext[0] == '#') {
                    ...
                    if (linetext.Left(2) == "##") {
                        ...
                        if (tag.Length == 2) {
                            // tag
                            switch (tag[0].Trim().ToLower()) {
                            case "##description":
                                sound.Description = tag[1];
                                break;
                            case "##instrument0":
                                if (byte.TryParse(tag[1], out temp)) {
                            ...
                            case "##instrument2":
                                ...
                                break;
                            case "##visible0":
                                if (bool.TryParse(tag[1], out value)) {
                                    sound[0].Visible = value;
                                }
                                break;
                            case "##visible1":
                                if (bool.TryParse(tag[1], out value)) {
                                    sound[1].Visible = value;
                                }
                                break;
                            case "##visible2":
                                if (bool.TryParse(tag[1], out value)) {
                                    sound[2].Visible = value;
                                }
                                break;
                            case "##visible3":
                                if (bool.TryParse(tag[1], out value)) {
                                    sound[3].Visible = value;
                                }
                                break;
                            case "##muted0":
                                if (bool.TryParse(tag[1], out value)) {
                ...
                else if (linetext.Equals("tone", StringComparison.OrdinalIgnoreCase)) {
                    track++;
                    // reset default note type
                    noteType = -1;
                }
                else if (linetext.Equals("noise", StringComparison.OrdinalIgnoreCase) && track != 3) {
                    track = 3;
                    // reset default note type
                    noteType = -1;
                }
                    ...
                    if (tag.Length >= 4) {
                        ...
                        if (track == 3) {
                            // 'p' or 'w' only
                            if (tag[0].Equals("p", StringComparison.OrdinalIgnoreCase)) {
                                noteType = 0;
                            }
                            else if (tag[0].Equals("w", StringComparison.OrdinalIgnoreCase)) {
                                noteType = 4;
                            }
                            else {
                                isError = true;
                            ...
                            frequency = (short)((uint)tag[1].Val() | (uint)noteType);
                        }
                        else {
                            ...
                            if (tag[0] == "a") {
                                ...
                                frequency = (short)((double)frequency / 111860);
                            }
                            else {
                                isError = true;
                                break;
                        ...
                        volume = (byte)tag[2].Val();
                        duration = tag[3].IntVal();
                    }
                        ...
                        if (track != 3) {
                            ...
                            frequency = (short)((uint)tag[0].Val() | (uint)noteType);
                        }
                        // get volume and duration
                        volume = (byte)tag[1].Val();
                        duration = tag[2].IntVal();
                    }
                    else {
                        isError = true;
                        break;
                    ...
                    // validate input
                    if (frequency < 0 || frequency >= 1024 || volume < 0 || volume >= 16 || duration < 0 || duration > 65535) {
                        isError = true;
                        break;
                    }
        ...
        public static void IT2AGI(string filename, Sound sound, SoundImportOptions options) {
            if (options.Channels is null)
                options.Channels = [1, 2, 3, 4];
            int NUMCH = options.Channels.Length;
            if (NUMCH > 4)
                throw new ArgumentException("IT2AGI only supports up to 4 channels.");
        ...
        public static void MOD2AGI(string filename, Sound sound, SoundImportOptions options) {
            ...
            int NUMCH = options.Channels.Length;
            if (NUMCH > 4)
                throw new ArgumentException("MOD2AGI only supports up to 4 channels.");
            ...
            if (mk != "M.K.")
                throw new InvalidDataException("Not a standard 4-channel MOD file.");
        ...
        public static void MIDI2AGI(string filename, Sound sound, SoundImportOptions options) {
            ...
            int NUMCH = options.Channels.Length;
            if (NUMCH > 4)
                throw new ArgumentException("MIDI2AGI only supports up to 4 channels.");
