...
namespace WinAGI.Editor {
    public partial class frmTextScreenEdit : ClipboardMonitor {
        ...
        private Bitmap BkgdPicture = null;
        private int PicOffset = -1;
        ...
        private readonly Pen dash1 = new(Color.Black), dash2 = new(Color.White);
        ...
        private void frmTextScreenEdit_HelpRequested(object sender, HelpEventArgs hlpevent) {
            ShowHelp();
        ...
        private void mnuEdit_DropDownOpening(object sender, EventArgs e) {
            mnuEdit.DropDownItems.AddRange([mnuMode, cmSep1, mnuCut,
                ...
                cmSep2, mnuCopyCommands, mnuPasteCommands, cmSep3,
                mnuCharMap, mnuToggleTextMarks, mnuChangeScreenSize,
                cmSep4, mnuAddBkgd, mnuRemoveBkgd, mnuPicOffset,
                cmSep5, mnuScale]);
            SetEditMenu();
        }
        ...
        private void SetEditMenu() {
            ...
            if (EditGame is null) {
                ...
                mnuAddBkgd.Enabled = true;
                if (BkgdPicture is null) {
                    mnuAddBkgd.Text = "Add Background Picture";
                }
                else {
        ...
        private void mnuMode_Click(object sender, EventArgs e) {
            ...
            // Overwrite mode, and is handled by the OnIdle method
            // of the main form (frmMDIMain).

            // !! ToggleInsertKey doesn't seem to be working anymore
        ...
        private void mnuAddBkgd_Click(object sender, EventArgs e) {
            if (EditGame is null) {
                return;
            }
        ...
        private void mnuRemoveBkgd_Click(object sender, EventArgs e) {
            if (BkgdPicNum == -1) {
                return;
            }
            BkgdPicture.Dispose();
        ...
        private void mnuPicOffset_Click(object sender, EventArgs e) {
            Form frm = new() {
                Text = "Set Picture Offset",
            ...
            frm.Controls.Add(new Label() {
                Text = "Picture Offset (0-4):",
                Location = new Point(10, 10),
                AutoSize = true
            });
            frm.Controls.Add(new NumericUpDown() {
                Name = "nudOffset",
                Minimum = 0,
                Maximum = 4,
                Value = PicOffset == -1 ? 1 : PicOffset,
        ...
        /// <summary>
        /// Draw the screen region bounded by rect.
        /// </summary>
        ...
        /// <summary>
        /// Draw the screen region bounded by the given parameters.
        /// </summary>
        ...
        private void DrawScreen(int left, int top, int width, int height) {
            ...
            if (BkgdPicture is not null) {
                // redraw bkd image in affected region
                DrawBkgd(left, top, width, height);
            }
            ...
            for (int j = top; j < top + height; j++) {
                for (int i = left; i < left + width; i++) {
                    if (TextData[i, j].CharVal > 0 && TextData[i, j].CharVal != 32) {
                        ...
                        if (TextData[i, j].CharVal == 32 || BkgdPicture is null ||
                            j < PicOffset || j >= PicOffset + 21) {
                            // or outside of picture
                            // 
        ...
        /// <summary>
        /// Refreshes the palette for this editor and redraws the screen.
        /// </summary>
        ...
        /// <summary>
        /// Clear entire draw surface to background color and draw the entire background image. 
        /// </summary>
        private void DrawBkgd() {
            int bWidth = (int)(320 * ScaleFactor), bHeight = (int)(200 * ScaleFactor);
            Bitmap bmp = new(bWidth, bHeight);
            ...
            gv.DrawImage(BkgdPicture, 0, PicOffset * 8 * ScaleFactor, bWidth, (int)(168 * ScaleFactor));
        }

        /// <summary>
        /// Clear only the specified region, and redraw that portion of the background image.
        /// </summary>
        /// <param name="left"></param>
        /// <param name="top"></param>
        /// <param name="width"></param>
        /// <param name="height"></param>
        private void DrawBkgd(int left, int top, int width, int height) {
            // redraw only the affected region (if on the picture image)
            if (top + height > PicOffset && top < PicOffset + 21) {
                // if any portion is off-pic, adjust the region
                if (top - PicOffset < 0) {
                    height -= PicOffset - top;
                    top = PicOffset;
                }
                if (top + height > 21 + PicOffset) {
                    height = 21 + PicOffset - top;
                }

                float srcX = left * CharWidth / 2, srcY = (top - PicOffset) * 8;
                float srcW = width * CharWidth / 2, srcH = height * 8;

                int tgtX = (int)(left * CharWidth * ScaleFactor), tgtY = (int)(top * 8 * ScaleFactor);
                int tgtW = (int)(width * CharWidth * ScaleFactor), tgtH = (int)(height * 8 * ScaleFactor);
                Rectangle tgtR = new(tgtX, tgtY, tgtW, tgtH);

                using Graphics g = Graphics.FromImage(picScreen.Image);
                g.InterpolationMode = InterpolationMode.NearestNeighbor;
                g.PixelOffsetMode = PixelOffsetMode.Half;
                g.DrawImage(BkgdPicture, tgtR, srcX, srcY, srcW, srcH, GraphicsUnit.Pixel);
            }
        }
        ...
        private void CopyAsCommands(Rectangle source) {
            ...
            for (int j = source.Top; j < source.Bottom; j++) {
                for (int i = source.Left; i < source.Right; i++) {
                    // if char is 255 (non-printable) convert it to space
                    if (TextData[i, j].CharVal == 255) {
            ...
            if (source.Width == ScreenWidth && source.Height == 25) {
                ...
                if (clearColor != AGIColorIndex.None) {
                    // foreground color is not needed, but use
                    // white if bkgd is non-white, otherwise use black
                    AGIColorIndex fg = AGIColorIndex.White;
                    if (clearColor == AGIColorIndex.White) {
                        fg = AGIColorIndex.Black;
                    }
                    output.Add("set.text.attribute(" + ColorText(fg) + ", " + ColorText(clearColor) + ");");
                }
                else {
                ...
                while (i < source.Right) {
                    if (newBG == clearColor) {
                        while (i < source.Right) {
                            ...
                            if (TextData[i, j].BG != newBG || (TextData[i, j].CharVal != 32 && TextData[i, j].CharVal != 0)) {
                    ...
                    while (i < source.Right) {
                        ...
                        if (TextData[i, j].BG != newBG || TextData[i, j].FG != newFG) {
                            ...
                            if (TextData[i, j].BG != newBG || (TextData[i, j].CharVal != 32 && TextData[i, j].CharVal != 0)) {
                                ...
                                if (linetext.Length > 0) {
                                    ...
                                    // add this line
                                    output.Add("display(" + dRow + ", " + dCol + ", " + "\"" + AddCodes(linetext) + "\"" + ");");
                                }
                                // update colors
                        ...
                        linetext += new string(Encoding.GetEncoding(ScreenCodePage).GetChars([charval]));
                                    ...
                                    while (mPos < TextIn.Count) {
                                        ...
                                        if (TextIn[mPos][0] == 'd') {
                                            ...
                                            if (iPos != mPos) {
                                                ...
                                                string temp = TextIn[mPos];
                                                TextIn.RemoveAt(mPos);
                                                TextIn.Insert(iPos, temp);
                                            }
                                            // move to next line
                                            mPos++;
                                            iPos++;
        ...
        private void PasteCommands(string pastetext) {
            ...
            AGIColorIndex fgcolor = DefFG;
            AGIColorIndex bgcolor = DefBG;
            ...
            AGIToken token = WinAGIFCTB.NextToken(pastetext, 0);
            while (token.Type != AGITokenType.None) {
                // only respond to "set.text.attribute", "display", "text.screen"
                switch (token.Text) {
                case "set.text.attribute":
                    // set.text.attribute(FG, BG)
                    token = WinAGIFCTB.NextToken(pastetext, token);
                    if (token.Text != "(") {
                        break;
                    }
                    token = WinAGIFCTB.NextToken(pastetext, token);
                    AGIColorIndex tmpFG = ColorFromToken(token);
                    if (tmpFG == AGIColorIndex.None) {
                        break;
                    }
                    // look for bg
                    token = WinAGIFCTB.NextToken(pastetext, token);
                    ...
                    token = WinAGIFCTB.NextToken(pastetext, token);
                    AGIColorIndex tmpBG = ColorFromToken(token);
                    token = WinAGIFCTB.NextToken(pastetext, token);
                    if (tmpBG == AGIColorIndex.None) {
                        break;
                    }
                ...
                case "display":
                    ...
                    token = WinAGIFCTB.NextToken(pastetext, token);
                    if (token.Type != AGITokenType.Number) {
                        break;
                    }
                    ...
                    string msg = AGIFormat(CompilerFormat(token.Text[1..^1]));
                    token = WinAGIFCTB.NextToken(pastetext, token);
                    if (token.Text != ")") {
                        break;
                    }
                    byte[] chars = Encoding.GetEncoding(ScreenCodePage).GetBytes(msg);
                    TextChar tmp = new() {
                        FG = fgcolor,
                        BG = bgcolor
                    };
            ...
            static AGIColorIndex ColorFromToken(AGIToken token) {
                if (token.Type == AGITokenType.Number) {
                    int value = int.Parse(token.Text);
                    if (value >= 0 && value <= 15) {
                        return (AGIColorIndex)value;
                    }
                }
                else if (token.Type == AGITokenType.Identifier &&
                        EditGame is not null && EditGame.IncludeReserved) {
                    // look for matching color name
                    AGIColorIndex tmp = AGIColorIndex.None;
                    for (int index = 0; index < 16; index++) {
                        if (token.Text == EditGame.ReservedDefines.ColorNames[index].Name) {
                            tmp = (AGIColorIndex)index;
                            break;
                        }
                    }
                    return tmp;
                ...
                return AGIColorIndex.None;
            }
        }

        private void AddText(Point insertpos, string text, AGIColorIndex backcolor, AGIColorIndex forecolor) {
            ...
            */
            ...
            byte[] textchars = Encoding.GetEncoding(ScreenCodePage).GetBytes(text);
            for (int i = 0; i < textchars.Length; i++) {
                ...
                case 9:
                    ...
                    for (int j = 1; j < WinAGISettings.LogicTabWidth.Value; j++) {
                        ...
                        TextData[insertpos.X, insertpos.Y].CharVal = 0;
                    }
                    break;
                case 10:
                case 13:
        ...
        private string CharPickFormat(string sInput) {
            ...
            // WinAGI editors, as it interferes with AGI's print methods.
        ...
        private void MarkAsSaved() {
            IsChanged = false;
            Text = "Text Screen Designer - " + CompactPath(FileName, 75);
