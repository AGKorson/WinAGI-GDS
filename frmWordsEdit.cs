...
*/
        ...
        public void MenuClickSave() {
          ...
          '  Dim blnDeleted As Boolean, lngGrp As Long
          '  Dim tmpNode As Node, tmpItem As ListItem

            On Error GoTo ErrHandler

            'autoupdate still needs significant work; disable it for now

          ''  'if in a game, and autoupdate is in ask mode or yes mode
          ''  If InGame And Settings.AutoUpdateWords <> 1 Then
          ''    'if not automatic,
          ''    If Settings.AutoUpdateWords = 0 Then
          ''      'get user's response
          ''      rtn = MsgBoxEx("Do you want to update all game logics with the changes made in the word list?", vbQuestion + vbYesNo, "Update Logics?", , , "Always take this action when saving the word list.", blnDontAsk)
          ''      If blnDontAsk Then
          ''        If rtn = vbYes Then
          ''          Settings.AutoUpdateWords = 2
          ''        Else
          ''          Settings.AutoUpdateWords = 1
          ''        End If
          ''      End If
          ''    Else
          ''      rtn = vbYes
          ...
          ''  End If
            ...
            If InGame Then
              ...
              'copy back to game
              On Error Resume Next
              VocabularyWords.SetWords WordsEdit
              'compile
              VocabularyWords.Save
              If Err.Number <> 0 Then
                ErrMsgBox "Error during WORDS.TOK compilation: ", "Existing WORDS.TOK has not been modified.", "WORDS.TOK Compile Error"
                Screen.MousePointer = vbDefault
                Exit Sub
              End If

              If blnRefreshLogics Then
                'mark all logics as dirty
                MakeAllDirty
                blnRefreshLogics = False
              End If

              'update properties window if words is currently selected
              UpdateSelection rtWords, -1, umProperty

              'clear any warnings
              frmMDIMain.ClearWarningList 0, rtWords
            ...
            Else
              'if no filename
              If LenB(WordsEdit.ResFile) = 0 Then
                MenuClickExport
                Exit Sub
              Else
                'show wait cursor
                ...
                Screen.MousePointer = vbDefault
                If Err.Number <> 0 Then
                  ErrMsgBox "An Error occurred while saving this wordlist:", "Existing wordlist file has not been modified.", "Wordlist Save Error"
                End If
                On Error GoTo ErrHandler
              End If
            End If
            ...
            If InGame Then
              Caption = Caption & GameID
            Else
              Caption = Caption & CompactPath(WordsEdit.ResFile, 75)
            End If
...
Public Sub MenuClickCustom2()
  ...
  'check for open logics- save or cancel if dirty
...
Private Sub RebuildGroupList(Optional ByVal SelGroup As Long = 0, Optional ByVal MarkDirty As Boolean = True)
  ...
  'file is dirty
  If MarkDirty And Not IsDirty Then
    MarkAsDirty
  End If
...
Private Sub ActivateActions()
 ...
 '*'Debug.Print "AdjustMenus 11"
  AdjustMenus rtWords, InGame, True, IsDirty
