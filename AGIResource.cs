...
namespace WinAGI.Engine {
    ...
    public class AGIResource {
        ...
        /// </returns>
        public ResourceErrorType Error {
            get; internal set;
        }
            ...
            set {
                ...
                if (NewID != mResID) {
                    ...
                    if (mInGame) {
                        ...
                        parent.WriteGameSetting(mResType.ToString() + Number, "ID", mResID, mResType.ToString() + "s", true);
                    }
                }
            }
        }
        ...
        public string Description {
            ...
            set {
                ...
                if (newDesc != mDescription) {
                    ...
                    if (mInGame) {
                        parent.WriteGameSetting(mResType.ToString() + Number, "Description", mDescription, mResType.ToString() + "s", true);
                    }
                }
            }
        }
        ...
        protected void InitInGame(AGIGame parent, AGIResType resType, byte resNum, sbyte VOL, int Loc) {
            ...
            if (mResID.Length == 0) {
                ...
                parent.WriteGameSetting(resType.ToString() + resNum, "ID", mResID, resType.ToString() + "s", true);
            }
        ...
        /// <exception cref="ArgumentOutOfRangeException"></exception>
        public void WriteWord(ushort InputInt, int Pos = -1, bool MSLS = false) {
            byte hiByte, loByte;
            ...
            hiByte = (byte)(InputInt >> 8);
            loByte = (byte)(InputInt % 256);
            if (MSLS) {
                mData[Pos] = hiByte;
                mData[Pos + 1] = loByte;
            }
        ...
        public ushort ReadWord(int Pos = -1, bool MSLS = false) {
            ...
            if (MSLS) {
                // high first
                hiByte = mData[mCurPos];
                loByte = mData[mCurPos + 1];
            }
            else {
                // low first
                loByte = mData[mCurPos];
                hiByte = mData[mCurPos + 1];
            }
            mCurPos += 2;
            mEORes = mCurPos == mData.Length;
        ...
        /// <exception cref="ArgumentOutOfRangeException"></exception>
        public void InsertData(dynamic data, int insertPos = -1) {
            int i, resEnd, newDataLength;
            byte[] insertData = [0];
            ...
            else if (data is byte newByte) {
                // single byte = OK
                insertData[0] = newByte;
            }
            else {
                // anything that can be converted to single byte = OK
                if (data is string) {
                    if (!byte.TryParse(data, out insertData[0])) {
                        throw new ArgumentException("Data Type Mismatch");
                    }
                }
                else {
                    try {
                        insertData[0] = (byte)data;
                    }
                    catch (Exception) {
                        throw new ArgumentException("Data Type Mismatch");
                    }
            ...
            resEnd = mData.Length;
            newDataLength = insertData.Length;
            ...
            if (insertPos != -1) {
                ...
                // insert pos is OK
                Array.Resize(ref mData, mData.Length + newDataLength);
                // move current data forward to make room for inserted data
                for (i = resEnd + newDataLength - 1; i >= insertPos + newDataLength; i--) {
                    mData[i] = mData[i - newDataLength];
                }
                // add newdata at insertpos
                for (i = 0; i < newDataLength; i++) {
                    mData[insertPos + i] = insertData[i];
                }
            }
            else {
                // insert at end
